...
$pc = !preg_match('~^(unsafe_raw)?$~', ini_get("filter.default"));
if ($pc || ini_get("filter.default_flags")) {
    foreach (array(
        '_GET',
        '_POST',
        '_COOKIE',
        ...
        $Dg = filter_input_array(constant("INPUT$X"), FILTER_UNSAFE_RAW);
        if ($Dg)
            $$X = $Dg;
    }
}
...
{
    $hd = substr($Kc, -1);
    return str_replace($hd . $hd, $hd, substr($Kc, 1, -1));
}
...
function remove_slashes($Qe, $pc = false)
{
    if (get_magic_quotes_gpc()) {
        while (list($z, $X) = each($Qe)) {
            foreach ($X as $ad => $W) {
                unset($Qe[$z][$ad]);
                if (is_array($W)) {
                    $Qe[$z][stripslashes($ad)] = $W;
                    $Qe[] =& $Qe[$z][stripslashes($ad)];
                } else
                    $Qe[$z][stripslashes($ad)] = ($pc ? $W : stripslashes($W));
            }
        }
    }
}
function bracket_escape($Kc, $_a = false)
{
    ...
    return strtr($Kc, ($_a ? array_flip($sg) : $sg));
}
function charset($e)
{
    return (version_compare($e->server_info, "5.5.3") >= 0 ? "utf8mb4" : "utf8");
}
function h($Mf)
{
    return str_replace("\0", "&#0;", htmlspecialchars($Mf, ENT_QUOTES, 'utf-8'));
}
function nbsp($Mf)
{
    return (trim($Mf) != "" ? h($Mf) : "&nbsp;");
...
function checkbox($E, $Y, $Na, $ed = "", $be = "", $Ra = "", $fd = "")
{
    $K = "<input type='checkbox' name='$E' value='" . h($Y) . "'" . ($Na ? " checked" : "") . ($fd ? " aria-labelledby='$fd'" : "") . ($be ? ' onclick="' . h($be) . '"' : '') . ">";
    return ($ed != "" || $Ra ? "<label" . ($Ra ? " class='$Ra'" : "") . ">$K" . h($ed) . "</label>" : $K);
}
function optionlist($fe, $uf = null, $Kg = false)
{
    $K = "";
    foreach ($fe as $ad => $W) {
        $ge = array(
        ...
        if (is_array($W)) {
            $K .= '<optgroup label="' . h($ad) . '">';
            $ge = $W;
        }
        foreach ($ge as $z => $X)
            $K .= '<option' . ($Kg || is_string($z) ? ' value="' . h($z) . '"' : '') . (($Kg || is_string($z) ? (string) $z : $X) === $uf ? ' selected' : '') . '>' . h($X);
        if (is_array($W))
            $K .= '</optgroup>';
    }
    return $K;
}
function html_select($E, $fe, $Y = "", $ae = true, $fd = "")
{
    if ($ae)
        return "<select name='" . h($E) . "'" . (is_string($ae) ? ' onchange="' . h($ae) . '"' : "") . ($fd ? " aria-labelledby='$fd'" : "") . ">" . optionlist($fe, $Y) . "</select>";
    $K = "";
    foreach ($fe as $z => $X)
...
function print_fieldset($u, $md, $Sg = false, $be = "")
{
    echo "<fieldset><legend><a href='#fieldset-$u' onclick=\"" . h($be) . "return !toggle('fieldset-$u');\">$md</a></legend><div id='fieldset-$u'" . ($Sg ? "" : " class='hidden'") . ">\n";
}
function bold($Ga, $Ra = "")
{
...
function js_escape($Mf)
{
    return addcslashes($Mf, "\r\n'\\/");
}
...
function set_password($Pg, $O, $V, $G)
{
    $_SESSION["pwds"][$Pg][$O][$V] = ($_COOKIE["adminer_key"] && is_string($G) ? array(
...
function get_key_vals($I, $f = null, $ig = 0)
{
    global $e;
    if (!is_object($f))
        $f = $e;
...
function where_link($t, $c, $Y, $ce = "=")
{
    return "&where%5B$t%5D%5Bcol%5D=" . urlencode($c) . "&where%5B$t%5D%5Bop%5D=" . urlencode(($Y !== null ? $ce : "IS NULL")) . "&where%5B$t%5D%5Bval%5D=" . urlencode($Y);
}
...
function cookie($E, $Y, $pd = 2592000)
{
    global $ba;
    return header("Set-Cookie: $E=" . urlencode($Y) . ($pd ? "; expires=" . gmdate("D, d M Y H:i:s", time() + $pd) . " GMT" : "") . "; path=" . preg_replace('~\\?.*~', '', $_SERVER["REQUEST_URI"]) . ($ba ? "; secure" : "") . "; HttpOnly; SameSite=lax");
}
...
function auth_url($Pg, $O, $V, $i = null)
{
    global $Fb;
    preg_match('~([^?]*)\\??(.*)~', remove_from_uri(implode("|", array_keys($Fb)) . "|username|" . ($i !== null ? "db|" : "") . session_name()), $C);
    return "$C[1]?" . (sid() ? SID . "&" : "") . ($Pg != "server" || $O != "" ? urlencode($Pg) . "=" . urlencode($O) . "&" : "") . "username=" . urlencode($V) . ($i != "" ? "&db=" . urlencode($i) : "") . ($C[2] ? "&$C[2]" : "");
}
function is_ajax()
{
...
function query_redirect($I, $B, $D, $Ye = true, $dc = true, $jc = false, $hg = "")
{
    global $e, $k, $b;
    if ($dc) {
        $If = microtime(true);
...
function apply_queries($I, $S, $Zb = 'table')
{
    foreach ($S as $Q) {
        if (!queries("$I " . $Zb($Q)))
            return false;
...
{
    list($Te, $hg) = queries(null);
    return query_redirect($Te, $B, $D, $Ye, false, !$Ye, $hg);
}
function format_time($If)
{
    return sprintf('%.3f s', max(0, microtime(true) - $If));
}
function remove_from_uri($te = "")
{
    return substr(preg_replace("~(?<=[?&])($te" . (SID ? "" : "|" . session_name()) . ")=[^&]*&~", '', "$_SERVER[REQUEST_URI]&"), 0, -1);
}
function pagination($F, $nb)
{
...
function get_file($z, $vb = false)
{
    ...
    foreach ($nc as $z => $X)
        $nc[$z] = (array) $X;
    $K = '';
    foreach ($nc["error"] as $z => $k) {
        if ($k)
            return $k;
        $E  = $nc["name"][$z];
        $pg = $nc["tmp_name"][$z];
        $fb = file_get_contents($vb && preg_match('~\\.gz$~', $E) ? "compress.zlib://$pg" : $pg);
        if ($vb) {
            $If = substr($fb, 0, 3);
            if (function_exists("iconv") && preg_match("~^\xFE\xFF|^\xFF\xFE~", $If, $ef))
                $fb = iconv("utf-16", "utf-8", $fb);
            elseif ($If == "\xEF\xBB\xBF")
                $fb = substr($fb, 3);
            $K .= $fb . "\n\n";
        } else
            $K .= $fb;
...
{
    $zd = ($k == UPLOAD_ERR_INI_SIZE ? ini_get("upload_max_filesize") : 0);
    return ($k ? 'Unable to upload a file.' . ($zd ? " " . sprintf('Maximum allowed file size is %sB.', $zd) : "") : 'File does not exist.');
}
function repeat_pattern($Be, $nd)
{
    return str_repeat("$Be{0,65535}", $nd / 65535) . "$Be{0," . ($nd % 65535) . "}";
}
function is_utf8($X)
{
    return (preg_match('~~u', $X) && !preg_match('~[\\0-\\x8\\xB\\xC\\xE-\\x1F]~', $X));
}
function shorten_utf8($Mf, $nd = 80, $Qf = "")
{
    if (!preg_match("(^(" . repeat_pattern("[\t\r\n -\x{10FFFF}]", $nd) . ")($)?)u", $Mf, $C))
        preg_match("(^(" . repeat_pattern("[\t\r\n -~]", $nd) . ")($)?)", $Mf, $C);
    return h($C[1]) . $Qf . (isset($C[2]) ? "" : "<i>...</i>");
}
...
function table_status1($Q, $kc = false)
{
    $K = table_status($Q, $kc);
    return ($K ? $K : array(
        "Name" => $Q
...
function enum_input($U, $wa, $l, $Y, $Tb = null)
{
    global $b;
    preg_match_all("~'((?:[^']|'')*)'~", $l["length"], $ud);
    $K = ($Tb !== null ? "<label><input type='$U'$wa value='$Tb'" . ((is_array($Y) ? in_array($Tb, $Y) : $Y === 0) ? " checked" : "") . "><i>" . 'empty' . "</i></label>" : "");
    foreach ($ud[1] as $t => $X) {
...
function input($l, $Y, $q)
{
    ...
    $gf = ($y == "mssql" && $l["auto_increment"]);
    if ($gf && !$_POST["save"])
        $q = null;
    $yc = (isset($_GET["select"]) || $gf ? array(
        "orig" => 'original'
    ) : array()) + $b->editFunctions($l);
    $wa = " name='fields[$E]'";
    if ($l["type"] == "enum")
        echo nbsp($yc[""]) . "<td>" . $b->editInput($_GET["edit"], $l, $wa, $Y);
    else {
        $qc = 0;
        foreach ($yc as $z => $X) {
            if ($z === "" || !$X)
        ...
        $ae = ($qc ? " onchange=\"var f = this.form['function[" . h(js_escape(bracket_escape($l["field"]))) . "]']; if ($qc > f.selectedIndex) f.selectedIndex = $qc;\" onkeyup='keyupChange.call(this);'" : "");
        $wa .= $ae;
        $Dc = (in_array($q, $yc) || isset($yc[$q]));
        echo (count($yc) > 1 ? "<select name='function[$E]' onchange='functionChange(this);'" . on_help("getTarget(event).value.replace(/^SQL\$/, '')", 1) . ">" . optionlist($yc, $q === null || $Dc ? $q : "") . "</select>" : nbsp(reset($yc))) . '<td>';
        $Qc = $b->editInput($_GET["edit"], $l, $wa, $Y);
        if ($Qc != "")
            echo $Qc;
        elseif (preg_match('~bool~', $l["type"]))
            echo "<input type='hidden'$wa value='0'>" . "<input type='checkbox'" . (in_array(strtolower($Y), array(
                '1',
                't',
                'true',
                'y',
        ...
        elseif ($l["type"] == "set") {
            ...
            foreach ($ud[1] as $t => $X) {
                ...
                echo " <label><input type='checkbox' name='fields[$E][$t]' value='" . (1 << $t) . "'" . ($Na ? ' checked' : '') . "$ae>" . h($b->editVal($X, $l)) . '</label>';
            }
        } elseif (preg_match('~blob|bytea|raw|file~', $l["type"]) && ini_bool("file_uploads"))
            echo "<input type='file' name='fields-$E'$ae>";
        elseif (($fg = preg_match('~text|lob~', $l["type"])) || preg_match("~\n~", $Y)) {
            if ($fg && $y != "sqlite")
                $wa .= " cols='50' rows='12'";
            else {
            ...
            $Ad = (!preg_match('~int~', $l["type"]) && preg_match('~^(\\d+)(,(\\d+))?$~', $l["length"], $C) ? ((preg_match("~binary~", $l["type"]) ? 2 : 1) * $C[1] + ($C[3] ? 1 : 0) + ($C[2] && !$l["unsigned"] ? 1 : 0)) : ($zg[$l["type"]] ? $zg[$l["type"]] + ($l["unsigned"] ? 0 : 1) : 0));
            if ($y == 'sql' && $e->server_info >= 5.6 && preg_match('~time~', $l["type"]))
                $Ad += 7;
            echo "<input" . ((!$Dc || $q === "") && preg_match('~(?<!o)int~', $l["type"]) && !preg_match('~\[\]~', $l["full_type"]) ? " type='number'" : "") . " value='" . h($Y) . "'" . ($Ad ? " data-maxlength='$Ad'" : "") . (preg_match('~char|binary~', $l["type"]) && $Ad > 20 ? " size='40'" : "") . "$wa>";
        }
    }
}
...
function dump_headers($Jc, $Hd = false)
{
    global $b;
    $K  = $b->dumpHeaders($Jc, $Hd);
    $re = $_POST["output"];
    if ($re != "text")
        header("Content-Disposition: attachment; filename=" . $b->dumpFilename($Jc) . ".$K" . ($re != "file" && !preg_match('~[^0-9a-z]~', $re) ? ".$re" : ""));
    session_write_close();
    ob_flush();
    flush();
    return $K;
...
{
    $oc = get_temp_dir() . "/adminer.key";
    $K  = @file_get_contents($oc);
    if ($K || !$g)
        return $K;
    $p = @fopen($oc, "w");
    if ($p) {
...
function select_value($X, $A, $l, $gg)
{
    global $b, $ba;
    if (is_array($X)) {
        $K = "";
        foreach ($X as $ad => $W)
            $K .= "<tr>" . ($X != array_values($X) ? "<th>" . h($ad) : "") . "<td>" . select_value($W, $A, $l, $gg);
    ...
    if ($A === null) {
        if (is_mail($X))
            $A = "mailto:$X";
        if ($Se = is_url($X))
            $A = (($Se == "http" && $ba) || preg_match('~WebKit|Firefox~i', $_SERVER["HTTP_USER_AGENT"]) ? $X : "https://www.adminer.org/redirect/?url=" . urlencode($X));
    }
    $K = $b->editVal($X, $l);
    if ($K !== null) {
        if ($K === "")
        ...
        elseif (!is_utf8($K))
            $K = "\0";
        elseif ($gg != "" && is_shortable($l))
            $K = shorten_utf8($K, max(0, +$gg));
        else
            $K = h($K);
    }
...
function is_mail($Qb)
{
    ...
    $Be = "$va+(\\.$va+)*@($Eb?\\.)+$Eb";
    return is_string($Qb) && preg_match("(^$Be(,\\s*$Be)*\$)i", $Qb);
}
function is_url($Mf)
{
    $Eb = '[a-z0-9]([-a-z0-9]{0,61}[a-z0-9])';
    return (preg_match("~^(https?)://($Eb?\\.)+$Eb(:\\d+)?(/.*)?(\\?.*)?(#.*)?\$~i", $Mf, $C) ? strtolower($C[1]) : "");
}
function is_shortable($l)
{
    return preg_match('~char|text|lob|geometry|point|linestring|polygon|string|bytea~', $l["type"]);
}
function count_rows($Q, $Z, $Wc, $s)
{
    global $y;
    $I = " FROM " . table($Q) . ($Z ? " WHERE " . implode(" AND ", $Z) : "");
    return ($Wc && ($y == "sql" || count($s) == 1) ? "SELECT COUNT(DISTINCT " . implode(", ", $s) . ")$I" : "SELECT COUNT(*)" . ($Wc ? " FROM (SELECT 1$I$_c) x" : $I));
}
...
function edit_form($a, $m, $L, $Gg)
{
    global $b, $y, $T, $k;
    $Vf = $b->tableName(table_status1($a, true));
    page_header(($Gg ? 'Edit' : 'Insert'), $k, array(
        "select" => array(
            $a,
            $Vf
        )
...
';
    if (!$m)
        ...
        foreach ($m as $E => $l) {
            ...
            if ($wb === null) {
                $wb = $l["default"];
                if ($l["type"] == "bit" && preg_match("~^b'([01]*)'\$~", $wb, $ef))
                    $wb = $ef[1];
            }
            $Y = ($L !== null ? ($L[$E] != "" && $y == "sql" && preg_match("~enum|set~", $l["type"]) ? (is_array($L[$E]) ? array_sum($L[$E]) : +$L[$E]) : $L[$E]) : (!$Gg && $l["auto_increment"] ? "" : (isset($_GET["select"]) ? false : $wb)));
            if (!$_POST["save"] && is_string($Y))
                $Y = $b->editVal($Y, $l);
            $q = ($_POST["save"] ? (string) $_POST["function"][$E] : ($Gg && $l["on_update"] == "CURRENT_TIMESTAMP" ? "now" : ($Y === false ? null : ($Y !== null ? '' : 'NULL'))));
        ...
        if (!support("table"))
            echo "<tr>" . "<th><input name='field_keys[]' onkeyup='keyupChange.call(this);' onchange='fieldChange(this);' value=''>" . "<td class='function'>" . html_select("field_funs[]", $b->editFunctions(array(
    ...
    if ($m) {
        ...
        if (!isset($_GET["select"]))
            echo "<input type='submit' name='insert' value='" . ($Gg ? 'Save and continue edit' . "' onclick='return !ajaxForm(this.form, \"" . 'Saving' . '...", this)' : 'Save and insert next') . "' title='Ctrl+Shift+Enter'>\n";
    }
    echo ($Gg ? "<input type='submit' name='delete' value='" . 'Delete' . "'" . confirm() . ">\n" : ($_POST || !$m ? "" : "<script type='text/javascript'>focus(document.getElementById('form').getElementsByTagName('td')[1].firstChild);</script>\n"));
    if (isset($_GET["select"]))
        hidden_fields(array(
...
global $b, $e, $Fb, $Mb, $Wb, $k, $yc, $Ac, $ba, $Pc, $y, $ca, $gd, $Zd, $Ce, $Nf, $Ec, $T, $ug, $zg, $Fg, $ga;
...
if (!defined("SID")) {
    ...
    if (version_compare(PHP_VERSION, '5.2.0') >= 0)
        $ue[] = true;
    call_user_func_array('session_set_cookie_params', $ue);
    session_start();
}
remove_slashes(array(
    &$_GET,
...
function lang($tg, $Qd = null)
{
    if (is_array($tg)) {
        $Fe = ($Qd == 1 ? 0 : 1);
        $tg = $tg[$Fe];
    }
    $tg = str_replace("%d", "%s", $tg);
    $Qd = format_number($Qd);
    return sprintf($tg, $Qd);
}
if (extension_loaded('pdo')) {
    class Min_PDO extends PDO
    {
        ...
        {
            global $b;
            $Fe = array_search("SQL", $b->operators);
            if ($Fe !== false)
                unset($b->operators[$Fe]);
        }
        function dsn($Jb, $V, $G)
        {
            try {
                parent::__construct($Jb, $V, $G);
            }
            catch (Exception $bc) {
...
class Min_SQL
{
    ...
    function select($Q, $N, $Z, $s, $he = array(), $_ = 1, $F = 0, $Me = false)
    {
        global $b, $y;
        $Wc = (count($s) < count($N));
        $I  = $b->selectQueryBuild($N, $Z, $s, $he, $_, $F);
        if (!$I)
            $I = "SELECT" . limit(($_GET["page"] != "last" && +$_ && $s && $Wc && $y == "sql" ? "SQL_CALC_FOUND_ROWS " : "") . implode(", ", $N) . "\nFROM " . table($Q), ($Z ? "\nWHERE " . implode(" AND ", $Z) : "") . ($s && $Wc ? "\nGROUP BY " . implode(", ", $s) : "") . ($he ? "\nORDER BY " . implode(", ", $he) : ""), ($_ != "" ? +$_ : null), ($F ? $_ * $F : 0), "\n");
        $If = microtime(true);
        $K  = $this->_conn->query($I);
        if ($Me)
            echo $b->selectQuery($I, format_time($If));
        return $K;
    }
    function delete($Q, $Ue, $_ = 0)
    {
        $I = "FROM " . table($Q);
        return queries("DELETE" . ($_ ? limit1($I, $Ue) : " $I$Ue"));
    }
    function update($Q, $P, $Ue, $_ = 0, $wf = "\n")
    {
        $Ng = array();
        foreach ($P as $z => $X)
            $Ng[] = "$z = $X";
        $I = table($Q) . " SET$wf" . implode(",$wf", $Ng);
        return queries("UPDATE" . ($_ ? limit1($I, $Ue) : " $I$Ue"));
    }
    function insert($Q, $P)
    {
        return queries("INSERT INTO " . table($Q) . ($P ? " (" . implode(", ", array_keys($P)) . ")\nVALUES (" . implode(", ", $P) . ")" : " DEFAULT VALUES"));
    }
    function insertUpdate($Q, $M, $Le)
    {
        return false;
    }
    function begin()
...
if (!defined("DRIVER")) {
    ...
    if (extension_loaded("mysqli")) {
        class Min_DB extends MySQLi
        {
            ...
            function connect($O = "", $V = "", $G = "", $rb = null, $Ee = null, $Ef = null)
            {
                mysqli_report(MYSQLI_REPORT_OFF);
                list($Hc, $Ee) = explode(":", $O, 2);
                $K = @$this->real_connect(($O != "" ? $Hc : ini_get("mysqli.default_host")), ($O . $V != "" ? $V : ini_get("mysqli.default_user")), ($O . $V . $G != "" ? $G : ini_get("mysqli.default_pw")), $rb, (is_numeric($Ee) ? $Ee : ini_get("mysqli.default_port")), (!is_numeric($Ee) ? $Ee : $Ef));
                return $K;
            }
        ...
        class Min_DB
        {
            ...
            function quote($Mf)
            {
                return "'" . mysql_real_escape_string($Mf, $this->_link) . "'";
            }
            function select_db($rb)
            {
                return mysql_select_db($rb, $this->_link);
            }
            function query($I, $_g = false)
            {
                $J           = @($_g ? mysql_unbuffered_query($I, $this->_link) : mysql_query($I, $this->_link));
                $this->error = "";
                if (!$J) {
        ...
        class Min_DB extends Min_PDO
        {
            ...
            function query($I, $_g = false)
            {
                $this->setAttribute(1000, !$_g);
                return parent::query($I, $_g);
            }
        }
    }
    class Min_Driver extends Min_SQL
    {
        ...
        function insertUpdate($Q, $M, $Le)
        {
            ...
            $Je = "INSERT INTO " . table($Q) . " (" . implode(", ", $d) . ") VALUES\n";
            $Ng = array();
            foreach ($d as $z)
                $Ng[$z] = "$z = VALUES($z)";
            ...
            foreach ($M as $P) {
                $Y = "(" . implode(", ", $P) . ")";
                if ($Ng && (strlen($Je) + $nd + strlen($Y) + strlen($Qf) > 1e6)) {
    ...
    function connect()
    {
        ...
        if ($e->connect($mb[0], $mb[1], $mb[2])) {
            ...
            if (version_compare($e->server_info, '5.7.8') >= 0) {
                $Nf['Strings'][] = "json";
                $zg["json"]      = 4294967295;
            }
            return $e;
    ...
    function get_databases($rc)
    {
        ...
        $K = get_session("dbs");
        if ($K === null) {
            $I = ($e->server_info >= 5 ? "SELECT SCHEMA_NAME FROM information_schema.SCHEMATA" : "SHOW DATABASES");
            $K = ($rc ? slow_query($I) : get_vals($I));
            restart_session();
            set_session("dbs", $K);
            stop_session();
        }
    ...
    {
        global $e;
        return get_key_vals($e->server_info >= 5 ? "SELECT TABLE_NAME, TABLE_TYPE FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() ORDER BY TABLE_NAME" : "SHOW TABLES");
    }
    ...
    function table_status($E = "", $kc = false)
    {
        global $e;
        $K = array();
        foreach (get_rows($kc && $e->server_info >= 5 ? "SELECT TABLE_NAME AS Name, ENGINE AS Engine, TABLE_COMMENT AS Comment FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() " . ($E != "" ? "AND TABLE_NAME = " . q($E) : "ORDER BY Name") : "SHOW TABLE STATUS" . ($E != "" ? " LIKE " . q(addcslashes($E, "%_\\")) : "")) as $L) {
            if ($L["Engine"] == "InnoDB")
    ...
    {
        global $e;
        return preg_match('~InnoDB|IBMDB2I~i', $R["Engine"]) || (preg_match('~NDB~i', $R["Engine"]) && version_compare($e->server_info, '5.6') >= 0);
    }
    function fields($Q)
    {
        $K = array();
    ...
    function foreign_keys($Q)
    {
        ...
        if ($kb) {
            preg_match_all("~CONSTRAINT ($Be) FOREIGN KEY ?\\(((?:$Be,? ?)+)\\) REFERENCES ($Be)(?:\\.($Be))? \\(((?:$Be,? ?)+)\\)(?: ON DELETE ($Zd))?(?: ON UPDATE ($Zd))?~", $kb, $ud, PREG_SET_ORDER);
            foreach ($ud as $C) {
                preg_match_all("~$Be~", $C[2], $Ff);
                preg_match_all("~$Be~", $C[5], $bg);
                $K[idf_unescape($C[1])] = array(
                    "db" => idf_unescape($C[4] != "" ? $C[3] : $C[4]),
                    "table" => idf_unescape($C[4] != "" ? $C[4] : $C[3]),
                    "source" => array_map('idf_unescape', $Ff[0]),
                    "target" => array_map('idf_unescape', $bg[0]),
    ...
    {
        global $e;
        return ($e->server_info >= 5 && $i == "information_schema") || ($e->server_info >= 5.5 && $i == "performance_schema");
    }
    function error()
    {
        global $e;
    ...
    function rename_database($E, $Wa)
    {
        ...
        if (create_database($E, $Wa)) {
            ...
            foreach (tables_list() as $Q => $U)
                $ff[] = table($Q) . " TO " . idf_escape($E) . "." . table($Q);
    ...
    function alter_table($Q, $E, $m, $sc, $bb, $Ub, $Wa, $ya, $ye)
    {
        $sa = array();
        foreach ($m as $l)
            $sa[] = ($l[1] ? ($Q != "" ? ($l[0] != "" ? "CHANGE " . idf_escape($l[0]) : "ADD") : " ") . " " . implode($l[1]) . ($Q != "" ? $l[2] : "") : "DROP " . idf_escape($l[0]));
        $sa = array_merge($sa, $sc);
        $Jf = ($bb !== null ? " COMMENT=" . q($bb) : "") . ($Ub ? " ENGINE=" . q($Ub) : "") . ($Wa ? " COLLATE " . q($Wa) : "") . ($ya != "" ? " AUTO_INCREMENT=$ya" : "");
        if ($Q == "")
        ...
        if ($Jf)
            $sa[] = ltrim($Jf);
        return ($sa || $ye ? queries("ALTER TABLE " . table($Q) . "\n" . implode(",\n", $sa) . $ye) : true);
    }
    ...
    function drop_views($Rg)
    {
        return queries("DROP VIEW " . implode(", ", array_map('table', $Rg)));
    }
    function drop_tables($S)
    {
        return queries("DROP TABLE " . implode(", ", array_map('table', $S)));
    }
    function move_tables($S, $Rg, $bg)
    {
        $ff = array();
        foreach (array_merge($S, $Rg) as $Q)
            $ff[] = table($Q) . " TO " . idf_escape($bg) . "." . table($Q);
        return queries("RENAME TABLE " . implode(", ", $ff));
    }
    function copy_tables($S, $Rg, $bg)
    {
        queries("SET sql_mode = 'NO_AUTO_VALUE_ON_ZERO'");
        foreach ($S as $Q) {
            $E = ($bg == DB ? table("copy_$Q") : idf_escape($bg) . "." . table($Q));
        ...
        foreach ($Rg as $Q) {
            $E  = ($bg == DB ? table("copy_$Q") : idf_escape($bg) . "." . table($Q));
            $Qg = view($Q);
            if (!queries("DROP VIEW IF EXISTS $E") || !queries("CREATE VIEW $E AS $Qg[select]"))
                return false;
        }
        return true;
    }
    ...
    function routine($E, $U)
    {
        ...
        $yg = "((" . implode("|", array_merge(array_keys($zg), $qa)) . ")\\b(?:\\s*\\(((?:[^'\")]|$Wb)++)\\))?\\s*(zerofill\\s*)?(unsigned(?:\\s+zerofill)?)?)(?:\\s*(?:CHARSET|CHARACTER\\s+SET)\\s*['\"]?([^'\"\\s,]+)['\"]?)?";
        $Be = "\\s*(" . ($U == "FUNCTION" ? "" : $Pc) . ")?\\s*(?:`((?:[^`]|``)*)`\\s*|\\b(\\S+)\\s+)$yg";
        $g  = $e->result("SHOW CREATE $U " . idf_escape($E), 2);
        preg_match("~\\(((?:$Be\\s*,?)*)\\)\\s*" . ($U == "FUNCTION" ? "RETURNS\\s+$yg\\s+" : "") . "(.*)~is", $g, $C);
        $m = array();
        preg_match_all("~$Be\\s*,?~is", $C[1], $ud, PREG_SET_ORDER);
        foreach ($ud as $te) {
            $E   = str_replace("``", "`", $te[2]) . $te[3];
            $m[] = array(
                "field" => $E,
                "type" => strtolower($te[5]),
                "length" => preg_replace_callback("~$Wb~s", 'normalize_enum', $te[6]),
                "unsigned" => strtolower(preg_replace('~\\s+~', ' ', trim("$te[8] $te[7]"))),
                "null" => 1,
                "full_type" => $te[4],
                "inout" => strtoupper($te[1]),
                "collation" => strtolower($te[9])
            );
        }
        if ($U != "FUNCTION")
            return array(
    ...
    {
        return $e->query("EXPLAIN " . ($e->server_info >= 5.1 ? "PARTITIONS " : "") . $I);
    }
    ...
    {
        global $e;
        return !preg_match("~scheme|sequence|type|view_trigger|materializedview" . ($e->server_info < 5.1 ? "|event|partitioning" . ($e->server_info < 5 ? "|routine|trigger|view" : "") : "") . "~", $lc);
    }
...
<tr><th>Server<td><input name="auth[server]" value="<?php
        echo h(SERVER), '" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
...
', "<p><input type='submit' value='" . 'Login' . "'>\n", checkbox("auth[permanent]", 1, $_COOKIE["adminer_permanent"], 'Permanent login') . "\n";
    ...
    function login($rd, $G)
    {
        global $y;
        if ($y == "sqlite")
            return sprintf('<a href="https://www.adminer.org/en/extension/" target="_blank">Implement</a> %s method to use SQLite.', '<code>login()</code>');
        return true;
    }
    function tableName($Uf)
    {
        return h($Uf["Name"]);
    }
    function fieldName($l, $he = 0)
    {
        return '<span title="' . h($l["full_type"]) . '">' . h($l["field"]) . '</span>';
    }
    function selectLinks($Uf, $P = "")
    {
        echo '<p class="links">';
        $qd = array(
            "select" => 'Select data'
        ...
        if (support("table") || support("indexes"))
            $qd["table"] = 'Show structure';
        if (support("table")) {
            if (is_view($Uf))
                $qd["view"] = 'Alter view';
            else
                $qd["create"] = 'Alter table';
        }
        if ($P !== null)
            $qd["edit"] = 'New item';
        foreach ($qd as $z => $X)
            echo " <a href='" . h(ME) . "$z=" . urlencode($Uf["Name"]) . ($z == "edit" ? $P : "") . "'" . bold(isset($_GET[$z])) . ">$X</a>";
        echo "\n";
    }
    ...
    function selectQuery($I, $hg)
    {
        global $y;
        return "<p><code class='jush-$y'>" . h(str_replace("\n", " ", $I)) . "</code> <span class='time'>($hg)</span>" . (support("sql") ? " <a href='" . h(ME) . "sql=" . urlencode($I) . "'>" . 'Edit' . "</a>" : "") . "</p>";
    }
    ...
    function selectVal($X, $A, $l, $pe)
    {
        $K = ($X === null ? "<i>NULL</i>" : (preg_match("~char|binary~", $l["type"]) && !preg_match("~var~", $l["type"]) ? "<code>$X</code>" : $X));
    ...
    function tableIndexesPrint($w)
    {
        ...
        foreach ($w as $E => $v) {
            ...
            foreach ($v["columns"] as $z => $X)
                $Me[] = "<i>" . h($X) . "</i>" . ($v["lengths"][$z] ? "(" . $v["lengths"][$z] . ")" : "") . ($v["descs"][$z] ? " DESC" : "");
            echo "<tr title='" . h($E) . "'><th>$v[type]<td>" . implode(", ", $Me) . "\n";
        }
        echo "</table>\n";
    }
    function selectColumnsPrint($N, $d)
    {
        ...
        foreach ($N as $z => $X) {
            $X = $_GET["columns"][$z];
            $c = select_input(" name='columns[$t][col]' onchange='" . ($z !== "" ? "selectFieldChange(this.form)" : "selectAddRow(this)") . ";'", $d, $X["col"]);
            echo "<div>" . ($yc || $Ac ? "<select name='columns[$t][fun]' onchange='helpClose();" . ($z !== "" ? "" : " this.nextSibling.nextSibling.onchange();") . "'" . on_help("getTarget(event).value && getTarget(event).value.replace(/ |\$/, '(') + ')'", 1) . ">" . optionlist(array(
                -1 => ""
            ) + array_filter(array(
                'Functions' => $yc,
                'Aggregation' => $Ac
            )), $X["fun"]) . "</select>" . "($c)" : $c) . "</div>\n";
            $t++;
        }
        echo "</div></fieldset>\n";
    }
    function selectSearchPrint($Z, $d, $w)
    {
        ...
        for ($t = 0; $t <= count($_GET["where"]); $t++) {
            ...
            if (!$X || ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators))) {
                echo "<div>" . select_input(" name='where[$t][col]' onchange='$Ka'", $d, $X["col"], "(" . 'anywhere' . ")"), html_select("where[$t][op]", $this->operators, $X["op"], $Ka), "<input type='search' name='where[$t][val]' value='" . h($X["val"]) . "' onchange='" . ($X ? "selectFieldChange(this.form)" : "selectAddRow(this)") . ";' onkeydown='selectSearchKeydown(this, event);' onsearch='selectSearchSearch(this);'></div>\n";
            }
        }
        echo "</div></fieldset>\n";
    }
    function selectOrderPrint($he, $d, $w)
    {
        print_fieldset("sort", 'Sort', $he);
        $t = 0;
        ...
        echo "<div>" . select_input(" name='order[$t]' onchange='selectAddRow(this);'", $d), checkbox("desc[$t]", 1, false, 'descending') . "</div>\n", "</div></fieldset>\n";
    }
    ...
    function selectLengthPrint($gg)
    {
        if ($gg !== null) {
            echo "<fieldset><legend>" . 'Text length' . "</legend><div>", "<input type='number' name='text_length' class='size' value='" . h($gg) . "'>", "</div></fieldset>\n";
        }
    }
    ...
    function selectEmailPrint($Rb, $d)
    {
    }
    function selectColumnsProcess($d, $w)
    {
        ...
        foreach ((array) $_GET["columns"] as $z => $X) {
            if ($X["fun"] == "count" || ($X["col"] != "" && (!$X["fun"] || in_array($X["fun"], $yc) || in_array($X["fun"], $Ac)))) {
                $N[$z] = apply_sql_function($X["fun"], ($X["col"] != "" ? idf_escape($X["col"]) : "*"));
                if (!in_array($X["fun"], $Ac))
                    $s[] = $N[$z];
            }
        }
        return array(
        ...
        foreach ((array) $_GET["where"] as $X) {
            if ("$X[col]$X[val]" != "" && in_array($X["op"], $this->operators)) {
                ...
                if ($X["col"] != "")
                    ...
                    $Ya = array();
                    foreach ($m as $E => $l) {
                        $Yc = preg_match('~char|text|enum|set~', $l["type"]);
                        if ((is_numeric($X["val"]) || !preg_match('~(^|[^o])int|float|double|decimal|bit~', $l["type"])) && (!preg_match("~[\x80-\xFF]~", $X["val"]) || $Yc)) {
                            $E    = idf_escape($E);
                            $Ya[] = ($y == "sql" && $Yc && !preg_match("~^utf8_~", $l["collation"]) ? "CONVERT($E USING " . charset($e) . ")" : $E);
                        }
                    }
    ...
    function selectEmailProcess($Z, $tc)
    {
        return false;
    }
    function selectQueryBuild($N, $Z, $s, $he, $_, $F)
    {
        return "";
    }
    function messageQuery($I, $hg)
    {
        global $y;
        restart_session();
        $Fc =& get_session("queries");
        $u = "sql-" . count($Fc[$_GET["db"]]);
        if (strlen($I) > 1e6)
        ...
        return " <span class='time'>" . @date("H:i:s") . "</span> <a href='#$u' onclick=\"return !toggle('$u');\">" . 'SQL command' . "</a>" . "<div id='$u' class='hidden'><pre><code class='jush-$y'>" . shorten_utf8($I, 1000) . '</code></pre>' . ($hg ? " <span class='time'>($hg)</span>" : '') . (support("sql") ? '<p><a href="' . h(str_replace("db=" . urlencode(DB), "db=" . urlencode($_GET["db"]), ME) . 'sql=&history=' . (count($Fc[$_GET["db"]]) - 1)) . '">' . 'Edit' . '</a>' : '') . '</div>';
    }
    ...
    function dumpData($Q, $Of, $I)
    {
        global $e, $y;
        $wd = ($y == "sqlite" ? 0 : 1048576);
        if ($Of) {
            if ($_POST["format"] == "sql") {
                if ($Of == "TRUNCATE+INSERT")
                    echo truncate_sql($Q) . ";\n";
                $m = fields($Q);
            }
            $J = $e->query($I, 1);
            if ($J) {
                ...
                while ($L = $J->$mc()) {
                    ...
                    if ($_POST["format"] != "sql") {
                        ...
                        if (!$Rc)
                            $Rc = "INSERT INTO " . table($Q) . " (" . implode(", ", array_map('idf_escape', $bd)) . ") VALUES";
                        foreach ($L as $z => $X) {
                            $l     = $m[$z];
                            $L[$z] = ($X !== null ? unconvert_field($l, preg_match('~(^|[^o])int|float|double|decimal~', $l["type"]) && $X != '' ? $X : q($X)) : "NULL");
                        }
                        $pf = ($wd ? "\n" : " ") . "(" . implode(",\t", $L) . ")";
                        if (!$Ia)
                            $Ia = $Rc . $pf;
                        elseif (strlen($Ia) + 4 + strlen($pf) + strlen($Qf) < $wd)
                            $Ia .= ",$pf";
                        else {
                            echo $Ia . $Qf;
                            $Ia = $Rc . $pf;
    ...
    function dumpFilename($Jc)
    {
        return friendly_url($Jc != "" ? $Jc : (SERVER != "" ? SERVER : "localhost"));
    }
    function dumpHeaders($Jc, $Hd = false)
    {
        $re = $_POST["output"];
        $hc = (preg_match('~sql~', $_POST["format"]) ? "sql" : ($Hd ? "tar" : "csv"));
        header("Content-Type: " . ($re == "gz" ? "application/x-gzip" : ($hc == "tar" ? "application/x-tar" : ($hc == "sql" || $re != "file" ? "text/plain" : "text/csv") . "; charset=utf-8")));
        if ($re == "gz")
...
';
        if ($Gd == "auth") {
            $qc = true;
            foreach ((array) $_SESSION["pwds"] as $Pg => $zf) {
                foreach ($zf as $O => $Lg) {
                    foreach ($Lg as $V => $G) {
                        if ($G !== null) {
                            if ($qc) {
                            ...
                            $ub = $_SESSION["db"][$Pg][$O][$V];
                            foreach (($ub ? array_keys($ub) : array(
                                ""
                            )) as $i)
                                echo "<a href='" . h(auth_url($Pg, $O, $V, $i)) . "'>($Fb[$Pg]) " . h($V . ($O != "" ? "@$O" : "") . ($i != "" ? " - $i" : "")) . "</a><br>\n";
                        }
                    }
                }
            }
            ...
            echo '<script type="text/javascript" src="', h(preg_replace("~\\?.*~", "", ME)) . "?file=jush.js&amp;version=4.3.0&amp;driver=mysql", '"></script>
';
            if (support("sql")) {
                echo '<script type="text/javascript">
';
                if ($S) {
                    ...
                    foreach ($S as $Q => $U)
                        $qd[] = preg_quote($Q, '/');
                    echo "var jushLinks = { $y: [ '" . js_escape(ME) . (support("table") ? "table=" : "select=") . "\$&', /\\b(" . implode("|", $qd) . ")\\b/g ] };\n";
                    foreach (array(
                        "bac",
                        "bra",
                        "sqlite_quo",
...
';
        ...
        echo "<span title='" . 'database' . "'>DB</span>: " . ($h ? "<select name='db'$sb>" . optionlist(array(
            "" => ""
        ) + $h, DB) . "</select>" : '<input name="db" value="' . h(DB) . '" autocapitalize="off">'), "<input type='submit' value='" . 'Use' . "'" . ($h ? " class='hidden'" : "") . ">\n";
        if ($Gd != "db" && DB != "" && $e->select_db(DB)) {
        }
    ...
    function tablesPrint($S)
    {
        ...
        foreach ($S as $Q => $Jf) {
            echo '<li><a href="' . h(ME) . 'select=' . urlencode($Q) . '"' . bold($_GET["select"] == $Q || $_GET["edit"] == $Q, "select") . ">" . 'select' . "</a> ";
            $E = $this->tableName($Jf);
            echo (support("table") || support("indexes") ? '<a href="' . h(ME) . 'table=' . urlencode($Q) . '"' . bold(in_array($Q, array(
                $_GET["table"],
                $_GET["create"],
                $_GET["indexes"],
                $_GET["foreign"],
                $_GET["trigger"]
            )), (is_view($Jf) ? "view" : "structure")) . " title='" . 'Show structure' . "'>$E</a>" : "<span>$E</span>") . "\n";
        }
        echo "</ul>\n";
    }
}
...
if ($b->operators === null)
    $b->operators = $de;
function page_header($kg, $k = "", $Ha = array(), $lg = "")
{
    global $ca, $ga, $b, $Fb, $y;
    page_headers();
    if (is_ajax() && $k) {
    ...
    $mg = $kg . ($lg != "" ? ": $lg" : "");
    $ng = strip_tags($mg . (SERVER != "" && SERVER != "localhost" ? h(" - " . SERVER) : "") . " - " . $b->name());
    echo '<!DOCTYPE html>
...
<title>', $ng, '</title>
<link rel="stylesheet" type="text/css" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=default.css&amp;version=4.3.0&amp;driver=mysql", '">
<script type="text/javascript" src="', h(preg_replace("~\\?.*~", "", ME)) . "?file=functions.js&amp;version=4.3.0&amp;driver=mysql", '"></script>
';
    if ($b->head()) {
        echo '<link rel="shortcut icon" type="image/x-icon" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=favicon.ico&amp;version=4.3.0&amp;driver=mysql", '">
<link rel="apple-touch-icon" href="', h(preg_replace("~\\?.*~", "", ME)) . "?file=favicon.ico&amp;version=4.3.0&amp;driver=mysql", '">
';
...
';
    if ($Ha !== null) {
        ...
        echo '<p id="breadcrumb"><a href="' . h($A ? $A : ".") . '">' . $Fb[DRIVER] . '</a> &raquo; ';
        $A = substr(preg_replace('~\b(db|ns)=[^&]*&~', '', ME), 0, -1);
        $O = (SERVER != "" ? h(SERVER) : 'Server');
        if ($Ha === false)
            echo "$O\n";
        else {
...
function long2str($W, $Tg)
{
    $pf = '';
    foreach ($W as $X)
        $pf .= pack('V', $X);
    if ($Tg)
        return substr($pf, 0, end($W));
    return $pf;
}
function str2long($pf, $Tg)
{
...
function xxtea_mx($Yg, $Xg, $Rf, $ad)
{
    return int32((($Yg >> 5 & 0x7FFFFFF) ^ $Xg << 2) + (($Xg >> 3 & 0x1FFFFFFF) ^ $Yg << 4)) ^ int32(($Rf ^ $Xg) + ($ad ^ $Yg));
}
function encrypt_string($Lf, $z)
{
    if ($Lf == "")
        return "";
    ...
    $Rf = 0;
    while ($H-- > 0) {
        $Rf = int32($Rf + 0x9E3779B9);
        $Lb = $Rf >> 2 & 3;
        for ($se = 0; $se < $Jd; $se++) {
            $Xg     = $W[$se + 1];
            $Id     = xxtea_mx($Yg, $Xg, $Rf, $z[$se & 3 ^ $Lb]);
            $Yg     = int32($W[$se] + $Id);
            $W[$se] = $Yg;
        }
        $Xg     = $W[0];
...
{
    global $b;
    $oc = get_temp_dir() . "/adminer.invalid";
    $p  = @fopen($oc, "r+");
    if (!$p) {
        $p = @fopen($oc, "w");
        if (!$p)
    ...
    flock($p, LOCK_EX);
    $Uc = unserialize(stream_get_contents($p));
    $hg = time();
    if ($Uc) {
        foreach ($Uc as $Vc => $X) {
            if ($X[0] < $hg)
...
if ($xa) {
    ...
    $i  = $xa["db"];
    set_password($Pg, $O, $V, $G);
    $_SESSION["db"][$Pg][$O][$V][$i] = true;
    if ($xa["permanent"]) {
        $z      = base64_encode($Pg) . "-" . base64_encode($O) . "-" . base64_encode($V) . "-" . base64_encode($i);
        $Ne     = $b->permanentLogin(true);
        $Ce[$z] = "$z:" . base64_encode($Ne ? encrypt_string($G, $Ne) : "");
        cookie("adminer_permanent", implode(" ", $Ce));
    }
    if (count($_POST) == 1 || DRIVER != $Pg || SERVER != $O || $_GET["username"] !== $V || DB != $i)
        redirect(auth_url($Pg, $O, $V, $i));
} elseif ($_POST["logout"]) {
    if ($Ec && !verify_token()) {
    ...
    foreach ($Ce as $z => $X) {
        list(, $Qa) = explode(":", $X);
        list($Pg, $O, $V, $i) = array_map('base64_decode', explode("-", $z));
        set_password($Pg, $O, $V, decrypt_string(base64_decode($Qa), $Ne));
        $_SESSION["db"][$Pg][$O][$V][$i] = true;
    }
}
function unset_permanent()
{
    ...
    foreach ($Ce as $z => $X) {
        list($Pg, $O, $V, $i) = array_map('base64_decode', explode("-", $z));
        if ($Pg == DRIVER && $O == SERVER && $V == $_GET["username"] && $i == DB)
            unset($Ce[$z]);
    }
    cookie("adminer_permanent", implode(" ", $Ce));
}
function auth_error($k)
{
    ...
    $ue = session_get_cookie_params();
    cookie("adminer_key", ($_COOKIE["adminer_key"] ? $_COOKIE["adminer_key"] : rand_string()), $ue["lifetime"]);
...
if (isset($_GET["username"])) {
    if (!class_exists("Min_DB")) {
        ...
        unset_permanent();
        page_header('No extension', sprintf('None of the supported PHP extensions (%s) are available.', implode(", ", $Ie)), false);
        page_footer("auth");
        exit;
    }
    $e = connect();
...
$j = new Min_Driver($e);
if (!is_object($e) || ($rd = $b->login($_GET["username"], get_password())) !== true)
    auth_error((is_string($e) ? h($e) : (is_string($rd) ? $rd : 'Invalid credentials.')));
if ($xa && $_POST["token"])
    $_POST["token"] = $T;
$k = '';
if ($_POST) {
    if (!verify_token()) {
        ...
        $k = (!$_POST["token"] && $_d ? sprintf('Maximum number of allowed fields exceeded. Please increase %s.', "'$Oc'") : 'Invalid CSRF token. Send the form again.' . ' ' . 'If you did not send this request from Adminer then close this page.');
    }
...
if (!ini_bool("session.use_cookies") || @ini_set("session.use_cookies", false) !== false)
    session_write_close();
function select($J, $f = null, $ke = array(), $_ = 0)
{
    global $y;
    $qd = array();
    $w  = array();
    ...
    for ($t = 0; (!$_ || $t < $_) && ($L = $J->fetch_row()); $t++) {
        if (!$t) {
            ...
            for ($x = 0; $x < count($L); $x++) {
                ...
                $K[$l->table] = $je;
                if ($ke && $y == "sql")
                    $qd[$x] = ($E == "table" ? "table=" : ($E == "possible_keys" ? "indexes=" : null));
                elseif ($je != "") {
                    if (!isset($w[$je])) {
                        $w[$je] = array();
                        foreach (indexes($je, $f) as $v) {
                            if ($v["type"] == "PRIMARY") {
                                $w[$je] = array_flip($v["columns"]);
                                break;
                            }
                        }
                        $d[$je] = $w[$je];
                ...
                if ($l->charsetnr == 63)
                    $Fa[$x] = true;
                $zg[$x] = $l->type;
                echo "<th" . ($je != "" || $l->name != $ie ? " title='" . h(($je != "" ? "$je." : "") . $ie) . "'" : "") . ">" . h($E) . ($ke ? doc_link(array(
        ...
        foreach ($L as $z => $X) {
            ...
            if (isset($qd[$z]) && !$d[$qd[$z]]) {
                if ($ke && $y == "sql") {
                    $Q = $L[array_search("table=", $qd)];
                    $A = $qd[$z] . urlencode($ke[$Q] != "" ? $ke[$Q] : $Q);
                } else {
                    $A = "edit=" . urlencode($qd[$z]);
                    foreach ($w[$qd[$z]] as $Ua => $x)
...
{
    $K = array();
    foreach (table_status('', true) as $Vf => $Q) {
        if ($Vf != $vf && fk_support($Q)) {
            foreach (fields($Vf) as $l) {
                if ($l["primary"]) {
                    if ($K[$Vf]) {
                        unset($K[$Vf]);
                        break;
...
function edit_type($z, $l, $Xa, $o = array())
{
    global $Nf, $zg, $Fg, $Zd;
    $U = $l["type"];
    ...
    if ($U && !isset($zg[$U]) && !isset($o[$U]))
        array_unshift($Nf, $U);
    if ($o)
        $Nf['Foreign keys'] = $o;
    echo optionlist($Nf, $U), '</select>
<td><input name="', h($z), '[length]" value="', h($l["length"]), '" size="3" onfocus="editingLengthFocus(this);"', (!$l["length"] && preg_match('~var(char|binary)$~', $U) ? " class='required'" : ""), ' onchange="editingLengthChange(this);" onkeyup="this.onchange();" aria-labelledby="label-length"><td class="options">';
    echo "<select name='" . h($z) . "[collation]'" . (preg_match('~(char|text|enum|set)$~', $U) ? "" : " class='hidden'") . '><option value="">(' . 'collation' . ')' . optionlist($Xa, $l["collation"]) . '</select>', ($Fg ? "<select name='" . h($z) . "[unsigned]'" . (!$U || preg_match('~((^|[^o])int|float|double|decimal)$~', $U) ? "" : " class='hidden'") . '><option>' . optionlist($Fg, $l["unsigned"]) . '</select>' : ''), (isset($l['on_update']) ? "<select name='" . h($z) . "[on_update]'" . (preg_match('~timestamp|datetime~', $U) ? "" : " class='hidden'") . '>' . optionlist(array(
        "" => "(" . 'ON UPDATE' . ")",
        "CURRENT_TIMESTAMP"
    ), $l["on_update"]) . '</select>' : ''), ($o ? "<select name='" . h($z) . "[on_delete]'" . (preg_match("~`~", $U) ? "" : " class='hidden'") . "><option value=''>(" . 'ON DELETE' . ")" . optionlist(explode("|", $Zd), $l["on_delete"]) . "</select> " : " ");
}
function process_length($nd)
{
    global $Wb;
    return (preg_match("~^\\s*\\(?\\s*$Wb(?:\\s*,\\s*$Wb)*+\\s*\\)?\\s*\$~", $nd) && preg_match_all("~$Wb~", $nd, $ud) ? "(" . implode(",", $ud[0]) . ")" : preg_replace('~^[0-9].*~', '(\0)', preg_replace('~[^-0-9,+()[\]]~', '', $nd)));
}
function process_type($l, $Va = "COLLATE")
{
    global $Fg;
    return " $l[type]" . process_length($l["length"]) . (preg_match('~(^|[^o])int|float|double|decimal~', $l["type"]) && in_array($l["unsigned"], $Fg) ? " $l[unsigned]" : "") . (preg_match('~char|text|enum|set~', $l["type"]) && $l["collation"] ? " $Va " . q($l["collation"]) : "");
}
function process_field($l, $xg)
{
    global $y;
    $wb = $l["default"];
    return array(
        ...
        (isset($wb) ? " DEFAULT " . ((preg_match('~time~', $l["type"]) && preg_match('~^CURRENT_TIMESTAMP$~i', $wb)) || ($y == "sqlite" && preg_match('~^CURRENT_(TIME|TIMESTAMP|DATE)$~i', $wb)) || ($l["type"] == "bit" && preg_match("~^([0-9]+|b'[0-1]+')\$~", $wb)) || ($y == "pgsql" && preg_match("~^[a-z]+\\(('[^']*')+\\)\$~", $wb)) ? $wb : q($wb)) : ""),
...
function edit_fields($m, $Xa, $U = "TABLE", $o = array(), $cb = false)
{
    global $e, $Pc;
    $m = array_values($m);
...
', (support("comment") ? "<td id='label-comment'" . ($cb ? "" : " class='hidden'") . ">" . 'Comment' : "");
    ...
    echo '<td>', "<input type='image' class='icon' name='add[" . (support("move_col") ? 0 : count($m)) . "]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.0&amp;driver=mysql' alt='+' title='" . 'Add next' . "'>", '<script type="text/javascript">row_count = ', count($m), ';</script>
</thead>
<tbody onkeydown="return editingKeydown(event);">
';
    foreach ($m as $t => $l) {
        $t++;
        $le = $l[($_POST ? "orig" : "field")];
        $Bb = (isset($_POST["add"][$t - 1]) || (isset($l["field"]) && !$_POST["drop_col"][$t])) && (support("drop_col") || $le == "");
        echo '<tr', ($Bb ? "" : " style='display: none;'"), '>
', ($U == "PROCEDURE" ? "<td>" . html_select("fields[$t][inout]", explode("|", $Pc), $l["inout"]) : ""), '<th>';
        if ($Bb) {
            echo '<input name="fields[', $t, '][field]" value="', h($l["field"]), '" onchange="editingNameChange(this);', ($l["field"] != "" || count($m) > 1 ? '' : ' editingAddRow(this);" onkeyup="if (this.value) editingAddRow(this);'), '" maxlength="64" autocapitalize="off" aria-labelledby="label-name">';
        }
        echo '<input type="hidden" name="fields[', $t, '][orig]" value="', h($le), '">
';
...
?> onclick="var field = this.form['fields[' + this.value + '][field]']; if (!field.value) { field.value = 'id'; field.onchange(); }" aria-labelledby="label-ai"></label><td><?php
            echo checkbox("fields[$t][has_default]", 1, $l["has_default"], "", "", "", "label-default"), '<input name="fields[', $t, '][default]" value="', h($l["default"]), '" onkeyup="keyupChange.call(this);" onchange="this.previousSibling.checked = true;" aria-labelledby="label-default">
', (support("comment") ? "<td" . ($cb ? "" : " class='hidden'") . "><input name='fields[$t][comment]' value='" . h($l["comment"]) . "' maxlength='" . ($e->server_info >= 5.5 ? 1024 : 255) . "' aria-labelledby='label-comment'>" : "");
        }
        echo "<td>", (support("move_col") ? "<input type='image' class='icon' name='add[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.0&amp;driver=mysql' alt='+' title='" . 'Add next' . "' onclick='return !editingAddRow(this, 1);'>&nbsp;" . "<input type='image' class='icon' name='up[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=up.gif&amp;version=4.3.0&amp;driver=mysql' alt='^' title='" . 'Move up' . "' onclick='return !editingMoveRow(this, 1);'>&nbsp;" . "<input type='image' class='icon' name='down[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=down.gif&amp;version=4.3.0&amp;driver=mysql' alt='v' title='" . 'Move down' . "' onclick='return !editingMoveRow(this, 0);'>&nbsp;" : ""), ($le == "" || support("drop_col") ? "<input type='image' class='icon' name='drop_col[$t]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=cross.gif&amp;version=4.3.0&amp;driver=mysql' alt='x' title='" . 'Remove' . "' onclick=\"return !editingRemoveRow(this, 'fields\$1[field]');\">" : ""), "\n";
    }
}
...
function grant($r, $Pe, $d, $Yd)
{
    ...
    if ($Pe == array(
        ...
        return ($r == "GRANT" ? queries("$r ALL PRIVILEGES$Yd WITH GRANT OPTION") : queries("$r ALL PRIVILEGES$Yd") && queries("$r GRANT OPTION$Yd"));
    return queries("$r " . preg_replace('~(GRANT OPTION)\\([^)]*\\)~', '\\1', implode("$d, ", $Pe) . $d) . $Yd);
}
function drop_create($Gb, $g, $Hb, $eg, $Ib, $B, $Dd, $Bd, $Cd, $Vd, $Md)
{
    if ($_POST["drop"])
        query_redirect($Gb, $B, $Dd);
    elseif ($Vd == "")
        query_redirect($g, $B, $Cd);
    elseif ($Vd != $Md) {
        $lb = queries($g);
        queries_redirect($B, $Bd, $lb && queries($Gb));
        if ($lb)
            queries($Hb);
    } else
        queries_redirect($B, $Bd, queries($eg) && queries($Ib) && queries($Gb) && queries($g));
}
function create_trigger($Yd, $L)
{
    global $y;
    $jg = " $L[Timing] $L[Event]" . ($L["Event"] == "UPDATE OF" ? " " . idf_escape($L["Of"]) : "");
    return "CREATE TRIGGER " . idf_escape($L["Trigger"]) . ($y == "mssql" ? $Yd . $jg : $jg . $Yd) . rtrim(" $L[Type]\n$L[Statement]", ";") . ";";
}
function create_routine($mf, $L)
{
    ...
    foreach ($m as $l) {
        if ($l["field"] != "")
            $P[] = (preg_match("~^($Pc)\$~", $l["inout"]) ? "$l[inout] " : "") . idf_escape($l["field"]) . process_type($l, "CHARACTER SET");
    }
...
{
    global $Zd;
    return " FOREIGN KEY (" . implode(", ", array_map('idf_escape', $n["source"])) . ") REFERENCES " . table($n["table"]) . " (" . implode(", ", array_map('idf_escape', $n["target"])) . ")" . (preg_match("~^($Zd)\$~", $n["on_delete"]) ? " ON DELETE $n[on_delete]" : "") . (preg_match("~^($Zd)\$~", $n["on_update"]) ? " ON UPDATE $n[on_update]" : "");
}
function tar_file($oc, $og)
{
    $K  = pack("a100a8a8a8a12a12", $oc, 644, 0, 0, decoct($og->size), decoct(time()));
    $Pa = 8 * 32;
    ...
    $og->send();
    echo str_repeat("\0", 511 - ($og->size + 511) % 512);
}
...
function doc_link($Ae)
{
    global $y, $e;
    $Ig = array(
        'sql' => "http://dev.mysql.com/doc/refman/" . substr($e->server_info, 0, 3) . "/en/",
        'sqlite' => "http://www.sqlite.org/",
        'pgsql' => "http://www.postgresql.org/docs/" . substr($e->server_info, 0, 3) . "/static/",
        'mssql' => "http://msdn.microsoft.com/library/",
        'oracle' => "http://download.oracle.com/docs/cd/B19306_01/server.102/b14200/"
    );
    return ($Ae[$y] ? "<a href='$Ig[$y]$Ae[$y]' target='_blank' rel='noreferrer'><sup>?</sup></a>" : "");
}
function ob_gzencode($Mf)
{
    return gzencode($Mf);
}
function db_size($i)
...
function connect_error()
{
    ...
    if (DB != "") {
        ...
        echo "<p>" . sprintf('%s version: %s through PHP extension %s', $Fb[DRIVER], "<b>" . h($e->server_info) . "</b>", "<b>$e->extension</b>") . "\n", "<p>" . sprintf('Logged as: %s', "<b>" . h(logged_user()) . "</b>") . "\n";
        $h = $b->databases();
        if ($h) {
            $sf = support("scheme");
            $Xa = collations();
            ...
            foreach ($h as $i => $S) {
                $lf = h(ME) . "db=" . urlencode($i);
                $u  = h("Db-" . $i);
                echo "<tr" . odd() . ">" . (support("database") ? "<td>" . checkbox("db[]", $i, in_array($i, (array) $_POST["db"]), "", "", "", $u) : ""), "<th><a href='$lf' id='$i'>" . h($i) . "</a>";
                $Wa = nbsp(db_collation($i, $Xa));
                echo "<td>" . (support("database") ? "<a href='$lf" . ($sf ? "&amp;ns=" : "") . "&amp;database=' title='" . 'Alter database' . "'>$Wa</a>" : $Wa), "<td align='right'><a href='$lf&amp;schema=' id='tables-" . h($i) . "' title='" . 'Database schema' . "'>" . ($_GET["dbsize"] ? $S : "?") . "</a>", "<td align='right' id='size-" . h($i) . "'>" . ($_GET["dbsize"] ? db_size($i) : "?"), "\n";
            }
...
if (!(DB != "" ? $e->select_db(DB) : isset($_GET["sql"]) || isset($_GET["dump"]) || isset($_GET["database"]) || isset($_GET["processlist"]) || isset($_GET["privileges"]) || isset($_GET["user"]) || isset($_GET["replication"]) || isset($_GET["variables"]) || $_GET["script"] == "connect" || $_GET["script"] == "kill")) {
    if (DB != "" || $_GET["refresh"]) {
        restart_session();
        set_session("dbs", null);
    }
...
$Zd = "RESTRICT|NO ACTION|CASCADE|SET NULL|SET DEFAULT";
class TmpFile
{
    var $handler;
    var $size;
...
if (isset($_GET["download"])) {
    ...
    page_header(($m && is_view($R) ? $R['Engine'] == 'materialized view' ? 'Materialized view' : 'View' : 'Table') . ": " . h($a), $k);
    $b->selectLinks($R);
    $bb = $R["Comment"];
    if ($bb != "")
        echo "<p>" . 'Comment' . ": " . h($bb) . "\n";
    ...
    preg_match_all('~([^:]+):([-0-9.]+)x([-0-9.]+)(_|$)~', $ea, $ud, PREG_SET_ORDER);
    foreach ($ud as $t => $C) {
        $Wf[$C[1]] = array(
            $C[2],
            $C[3]
    ...
    foreach (table_status('', true) as $Q => $R) {
        ...
        foreach ($b->foreignKeys($Q) as $X) {
            if (!$X["db"]) {
                $jd = $Ca;
                if ($Wf[$Q][1] || $Wf[$X["table"]][1])
                    $jd = min(floatval($Wf[$Q][1]), floatval($Wf[$X["table"]][1])) - 1;
                else
                    $Ca -= .1;
                while ($ld[(string) $jd])
                    $jd -= .0001;
                $rf[$Q]["references"][$X["table"]][(string) $jd] = array(
                    $X["source"],
                    $X["target"]
                );
                $cf[$X["table"]][$Q][(string) $jd]               = $X["target"];
                $ld[(string) $jd]                                = true;
            }
        }
        $qg = max($qg, $rf[$Q]["pos"][0] + 2.5 + $Fe);
    }
    echo '<div id="schema" style="height: ', $qg, 'em;" onselectstart="return false;">
<script type="text/javascript">
...
';
    foreach ($rf as $E => $Q) {
        ...
        foreach ((array) $Q["references"] as $cg => $df) {
            foreach ($df as $jd => $Ze) {
                $kd = $jd - $Wf[$E][1];
                $t  = 0;
                foreach ($Ze[0] as $Ff)
                    echo "\n<div class='references' title='" . h($cg) . "' id='refs$jd-" . ($t++) . "' style='left: $kd" . "em; top: " . $Q["fields"][$Ff]["pos"] . "em; padding-top: .5em;'><div style='border-top: 1px solid Gray; width: " . (-$kd) . "em;'></div></div>";
            }
        }
        foreach ((array) $cf[$E] as $cg => $df) {
            foreach ($df as $jd => $d) {
                $kd = $jd - $Wf[$E][1];
                $t  = 0;
                foreach ($d as $bg)
                    echo "\n<div class='references' title='" . h($cg) . "' id='refd$jd-" . ($t++) . "' style='left: $kd" . "em; top: " . $Q["fields"][$bg]["pos"] . "em; height: 1.25em; background: url(" . h(preg_replace("~\\?.*~", "", ME)) . "?file=arrow.gif) no-repeat right center;&amp;version=4.3.0&amp;driver=mysql'><div style='height: .5em; border-bottom: 1px solid Gray; width: " . (-$kd) . "em;'></div></div>";
            }
        }
        echo "\n</div>\n";
    }
    foreach ($rf as $E => $Q) {
        foreach ((array) $Q["references"] as $cg => $df) {
            foreach ($df as $jd => $Ze) {
                $Fd = $qg;
                $yd = -10;
                foreach ($Ze[0] as $z => $Ff) {
                    $Ge = $Q["pos"][0] + $Q["fields"][$Ff]["pos"];
                    $He = $rf[$cg]["pos"][0] + $rf[$cg]["fields"][$Ze[1][$z]]["pos"];
                    $Fd = min($Fd, $Ge, $He);
                    $yd = max($yd, $Ge, $He);
                }
                echo "<div class='references' id='refl$jd' style='left: $jd" . "em; top: $Fd" . "em; padding: .5em 0;'><div style='border-right: 1px solid Gray; margin-top: 1px; height: " . ($yd - $Fd) . "em;'></div></div>\n";
            }
        }
    }
    echo '</div>
...
';
    ...
    if ($_POST && !$k) {
        ...
        $hc = dump_headers((count($S) == 1 ? key($S) : DB), (DB == "" || count($S) > 1));
        $Xc = preg_match('~sql~', $_POST["format"]);
        if ($Xc) {
            echo "-- Adminer $ga " . $Fb[DRIVER] . " dump\n\n";
            if ($y == "sql") {
                echo "SET NAMES utf8;
SET time_zone = '+00:00';
...
";
        ...
        foreach ((array) $h as $i) {
            $b->dumpDatabase($i);
            if ($e->select_db($i)) {
                if ($Xc && preg_match('~CREATE~', $Of) && ($g = $e->result("SHOW CREATE DATABASE " . idf_escape($i), 1))) {
                    set_utf8mb4($g);
                    if ($Of == "DROP+CREATE")
                ...
                if ($Xc) {
                    ...
                    if ($_POST["routines"]) {
                        foreach (array(
                            ...
                            foreach (get_rows("SHOW $mf STATUS WHERE Db = " . q($i), null, "-- ") as $L) {
                                $g = remove_definer($e->result("SHOW CREATE $mf " . idf_escape($L["Name"]), 2));
                                set_utf8mb4($g);
                                $qe .= ($Of != 'DROP+CREATE' ? "DROP $mf IF EXISTS " . idf_escape($L["Name"]) . ";;\n" : "") . "$g;;\n\n";
                            }
                        }
                    }
                    if ($_POST["events"]) {
                        foreach (get_rows("SHOW EVENTS", null, "-- ") as $L) {
                            ...
                            set_utf8mb4($g);
                            $qe .= ($Of != 'DROP+CREATE' ? "DROP EVENT IF EXISTS " . idf_escape($L["Name"]) . ";;\n" : "") . "$g;;\n\n";
                        }
                    }
                    if ($qe)
                        echo "DELIMITER ;;\n\n$qe" . "DELIMITER ;\n\n";
                ...
                if ($_POST["table_style"] || $_POST["data_style"]) {
                    ...
                    foreach (table_status('', true) as $E => $R) {
                        ...
                        if ($Q || $pb) {
                            ...
                            if ($Xc && $_POST["triggers"] && $Q && ($wg = trigger_sql($E, $_POST["table_style"])))
                                echo "\nDELIMITER ;;\n$wg\nDELIMITER ;\n";
                            if ($hc == "tar") {
                                ob_end_flush();
                                tar_file((DB != "" ? "" : "$i/") . "$E.csv", $og);
                            } elseif ($Xc)
                                echo "\n";
                        }
                    }
                    foreach ($Rg as $Qg)
                        $b->dumpTable($Qg, $_POST["table_style"], 1);
                    if ($hc == "tar")
                        echo pack("x512");
                }
            }
...
';
    ...
    echo ($y == "sqlite" ? "" : "<tr><th>" . 'Database' . "<td>" . html_select('db_style', $tb, $L["db_style"]) . (support("routine") ? checkbox("routines", 1, $L["routines"], 'Routines') : "") . (support("event") ? checkbox("events", 1, $L["events"], 'Events') : "")), "<tr><th>" . 'Tables' . "<td>" . html_select('table_style', $Yf, $L["table_style"]) . checkbox("auto_increment", 1, $L["auto_increment"], 'Auto Increment') . (support("trigger") ? checkbox("triggers", 1, $L["triggers"], 'Triggers') : ""), "<tr><th>" . 'Data' . "<td>" . html_select('data_style', $qb, $L["data_style"]), '</table>
...
';
    ...
    if (DB != "") {
        $Na = ($a != "" ? "" : " checked");
        echo "<thead><tr>", "<th style='text-align: left;'><label class='block'><input type='checkbox' id='check-tables'$Na onclick='formCheck(this, /^tables\\[/);'>" . 'Tables' . "</label>", "<th style='text-align: right;'><label class='block'>" . 'Data' . "<input type='checkbox' id='check-data'$Na onclick='formCheck(this, /^data\\[/);'></label>", "</thead>\n";
        $Rg = "";
        $Zf = tables_list();
        foreach ($Zf as $E => $U) {
            $Je = preg_replace('~_.*~', '', $E);
            $Na = ($a == "" || $a == (substr($a, -1) == "%" ? "$Je%" : $E));
            $Me = "<tr><td>" . checkbox("tables[]", $E, $Na, $E, "checkboxClick(event, this); formUncheck('check-tables');", "block");
        ...
        if ($h) {
            foreach ($h as $i) {
                if (!information_schema($i)) {
                    $Je = preg_replace('~_.*~', '', $i);
                    echo "<tr><td>" . checkbox("databases[]", $i, $a == "" || $a == "$Je%", $i, "formUncheck('check-databases');", "block") . "\n";
                    $Ke[$Je]++;
                }
            }
        } else
...
';
    ...
    foreach ($Ke as $z => $X) {
        if ($z != "" && $X > 1) {
            echo ($qc ? "<p>" : " ") . "<a href='" . h(ME) . "dump=" . urlencode("$z%") . "'>" . h($z) . "</a>";
            $qc = false;
        }
    }
    ...
    if (!$k && $_POST) {
        ...
        elseif ($_POST["webfile"]) {
            $p = @fopen((file_exists("adminer.sql") ? "adminer.sql" : "compress.zlib://adminer.sql.gz"), "rb");
        ...
        if (is_string($I)) {
            ...
            $ab = 0;
            $Yb = array();
            $ve = '[\'"' . ($y == "sql" ? '`#' : ($y == "sqlite" ? '`[' : ($y == "mssql" ? '[' : ''))) . ']|/\\*|-- |$' . ($y == "pgsql" ? '|\\$[^$]*\\$' : '');
            $rg = microtime(true);
            parse_str($_COOKIE["adminer_export"], $la);
            $Kb = $b->dumpFormat();
            unset($Kb["sql"]);
            while ($I != "") {
                if (!$Sd && preg_match("~^$Gf*+DELIMITER\\s+(\\S+)~i", $I, $C)) {
                    $xb = $C[1];
                    $I  = substr($I, strlen($C[0]));
                } else {
                    preg_match('(' . preg_quote($xb) . "\\s*|$ve)", $I, $C, PREG_OFFSET_CAPTURE, $Sd);
                    list($vc, $Fe) = $C[0];
                    if (!$vc && $p && !feof($p))
                        $I .= fread($p, 1e5);
                    else {
                        ...
                        $Sd = $Fe + strlen($vc);
                        if ($vc && rtrim($vc) != $xb) {
                            while (preg_match('(' . ($vc == '/*' ? '\\*/' : ($vc == '[' ? ']' : (preg_match('~^-- |^#~', $vc) ? "\n" : preg_quote($vc) . "|\\\\."))) . '|$)s', $I, $C, PREG_OFFSET_CAPTURE, $Sd)) {
                                $pf = $C[0][0];
                                if (!$pf && $p && !feof($p))
                                    $I .= fread($p, 1e5);
                                else {
                            ...
                            $H  = substr($I, 0, $Fe);
                            $ab++;
                            $Me = "<pre id='sql-$ab'><code class='jush-$y'>" . $b->sqlCommandQuery($H) . "</code></pre>\n";
                            if ($y == "sqlite" && preg_match("~^$Gf*+ATTACH\\b~i", $H, $C)) {
                                echo $Me, "<p class='error'>" . 'ATTACH queries are not supported.' . "\n";
                                $Yb[] = " <a href='#sql-$ab'>$ab</a>";
                                ...
                                $If = microtime(true);
                                if ($e->multi_query($H) && is_object($f) && preg_match("~^$Gf*+USE\\b~i", $H))
                                    $f->query($H);
                                do {
                                    $J  = $e->store_result();
                                    $hg = " <span class='time'>(" . format_time($If) . ")</span>" . (strlen($H) < 1000 ? " <a href='" . h(ME) . "sql=" . urlencode(trim($H)) . "'>" . 'Edit' . "</a>" : "");
                                    if ($e->error) {
                                        echo ($_POST["only_errors"] ? $Me : ""), "<p class='error'>" . 'Error in query' . ($e->errno ? " ($e->errno)" : "") . ": " . error() . "\n";
                                        $Yb[] = " <a href='#sql-$ab'>$ab</a>";
                                        if ($_POST["error_stops"])
                                            break 2;
                                    } elseif (is_object($J)) {
                                        $_  = $_POST["limit"];
                                        $ke = select($J, $f, array(), $_);
                                        if (!$_POST["only_errors"]) {
                                            echo "<form action='' method='post'>\n";
                                            $Pd = $J->num_rows;
                                            echo "<p>" . ($Pd ? ($_ && $Pd > $_ ? sprintf('%d / ', $_) : "") . lang(array(
                                                '%d row',
                                                '%d rows'
                                            ), $Pd) : ""), $hg;
                                            $u  = "export-$ab";
                                            $gc = ", <a href='#$u' onclick=\"return !toggle('$u');\">" . 'Export' . "</a><span id='$u' class='hidden'>: " . html_select("output", $b->dumpOutput(), $la["output"]) . " " . html_select("format", $Kb, $la["format"]) . "<input type='hidden' name='query' value='" . h($H) . "'>" . " <input type='submit' name='export' value='" . 'Export' . "'><input type='hidden' name='token' value='$T'></span>\n";
                                            if ($f && preg_match("~^($Gf|\\()*+SELECT\\b~i", $H) && ($fc = explain($f, $H))) {
                                                $u = "explain-$ab";
                                                echo ", <a href='#$u' onclick=\"return !toggle('$u');\">EXPLAIN</a>$gc", "<div id='$u' class='hidden'>\n";
                                                select($fc, $f, $ke);
                                                echo "</div>\n";
                                            } else
                                                echo $gc;
                                        ...
                                        if (preg_match("~^$Gf*+(CREATE|DROP|ALTER)$Gf++(DATABASE|SCHEMA)\\b~i", $H)) {
                                            restart_session();
                                            set_session("dbs", null);
                                            stop_session();
                                        }
            ...
            elseif ($_POST["only_errors"]) {
                ...
                ), $ab - count($Yb)), " <span class='time'>(" . format_time($rg) . ")</span>\n";
            } elseif ($Yb && $ab > 1)
                echo "<p class='error'>" . 'Error in query' . ": " . implode("", $Yb) . "\n";
        } else
...
';
    $dc = "<input type='submit' value='" . 'Execute' . "' title='Ctrl+Enter'>";
    if (!isset($_GET["import"])) {
        $H = $_GET["sql"];
        if ($_POST)
            $H = $_POST["query"];
    ...
    if (!isset($_GET["import"]) && $Fc) {
        ...
        for ($X = end($Fc); $X; $X = prev($Fc)) {
            $z = key($Fc);
            list($H, $hg, $Ob) = $X;
            echo '<a href="' . h(ME . "sql=&history=$z") . '">' . 'Edit' . "</a>" . " <span class='time' title='" . @date('Y-m-d', $hg) . "'>" . @date("H:i:s", $hg) . "</span>" . " <code class='jush-$y'>" . shorten_utf8(ltrim(str_replace("\n", " ", str_replace("\r", "", preg_replace('~^(#|-- ).*~m', '', $H)))), 80, "</code>") . ($Ob ? " <span class='time'>($Ob)</span>" : "") . "<br>\n";
        }
...
';
    ...
    $a  = $_GET["edit"];
    $m  = fields($a);
    $Z  = (isset($_GET["select"]) ? (count($_POST["check"]) == 1 ? where_check($_POST["check"][0], $m) : "") : where($_GET, $m));
    $Gg = (isset($_GET["select"]) ? $_POST["edit"] : $Z);
    foreach ($m as $E => $l) {
        if (!isset($l["privileges"][$Gg ? "update" : "insert"]) || $b->fieldName($l) == "")
            unset($m[$E]);
    }
    if ($_POST && !$k && !isset($_GET["select"])) {
        $B = $_POST["referer"];
        if ($_POST["insert"])
            $B = ($Gg ? null : $_SERVER["REQUEST_URI"]);
        elseif (!preg_match('~^.+&select=.+$~', $B))
            $B = ME . "select=" . urlencode($a);
        $w  = indexes($a);
        $Bg = unique_array($_GET["where"], $w);
        $Ve = "\nWHERE $Z";
        if (isset($_POST["delete"]))
            queries_redirect($B, 'Item has been deleted.', $j->delete($a, $Ve, !$Bg));
        else {
            $P = array();
            foreach ($m as $E => $l) {
                $X = process_input($l);
            ...
            if ($Gg) {
                if (!$P)
                    redirect($B);
                queries_redirect($B, 'Item has been updated.', $j->update($a, $P, $Ve, !$Bg));
                if (is_ajax()) {
                    page_headers();
                    page_messages($k);
                    exit;
                ...
                $J  = $j->insert($a, $P);
                $id = ($J ? last_id() : 0);
                queries_redirect($B, sprintf('Item%s has been inserted.', ($id ? " $id" : "")), $J);
            }
        }
    }
    $L = null;
    ...
    foreach (array(
        ...
        $we[$z] = $z;
    $bf = referencable_primary($a);
    $o  = array();
    foreach ($bf as $Vf => $l)
        $o[str_replace("`", "``", $Vf) . "`" . str_replace("`", "``", $l["field"])] = $Vf;
    $ne = array();
    $R  = array();
    if ($a != "") {
        $ne = fields($a);
        ...
        if ($_POST["drop"])
            ...
            foreach ($L["fields"] as $z => $l) {
                ...
                if ($l["field"] != "") {
                    ...
                    if ($n !== null)
                        $sc[idf_escape($l["field"])] = ($a != "" && $y != "sqlite" ? "ADD" : " ") . format_foreign_key(array(
                            'table' => $o[$l["type"]],
                            'source' => array(
                                $l["field"]
                            ),
            ...
            $E = trim($L["name"]);
            queries_redirect(ME . (support("table") ? "table=" : "select=") . urlencode($E), $D, alter_table($a, $E, ($y == "sqlite" && ($Jg || $sc) ? $ra : $m), $sc, ($L["Comment"] != $R["Comment"] ? $L["Comment"] : null), ($L["Engine"] && $L["Engine"] != $R["Engine"] ? $L["Engine"] : ""), ($L["Collation"] && $L["Collation"] != $R["Collation"] ? $L["Collation"] : ""), ($L["Auto_increment"] != "" ? number($L["Auto_increment"]) : ""), $ye));
        }
    }
    ...
    if (!$_POST) {
        $L = array(
            ...
            "fields" => array(
                array(
                    "field" => "",
                    "type" => (isset($zg["int"]) ? "int" : (isset($zg["integer"]) ? "integer" : ""))
                )
            ),
        ...
        if ($a != "") {
            ...
            if (support("partitioning")) {
                $xc = "FROM information_schema.PARTITIONS WHERE TABLE_SCHEMA = " . q(DB) . " AND TABLE_NAME = " . q($a);
                $J  = $e->query("SELECT PARTITION_METHOD, PARTITION_ORDINAL_POSITION, PARTITION_EXPRESSION $xc ORDER BY PARTITION_ORDINAL_POSITION DESC LIMIT 1");
                list($L["partition_by"], $L["partitions"], $L["partition"]) = $J->fetch_row();
                $ze                    = get_key_vals("SELECT PARTITION_NAME, PARTITION_DESCRIPTION $xc AND PARTITION_NAME != '' ORDER BY PARTITION_ORDINAL_POSITION");
                $ze[""]                = "";
                $L["partition_names"]  = array_keys($ze);
                $L["partition_values"] = array_values($ze);
            }
        }
    }
    $Xa = collations();
...
?><script type='text/javascript'>focus(document.getElementById('form')['name']);</script><?php
        ...
        echo ($Vb ? "<select name='Engine' onchange='helpClose();'" . on_help("getTarget(event).value", 1) . ">" . optionlist(array(
            "" => "(" . 'engine' . ")"
        ) + $Vb, $L["Engine"]) . "</select>" : ""), ' ', ($Xa && !preg_match("~sqlite|mssql~", $y) ? html_select("Collation", array(
            "" => "(" . 'collation' . ")"
...
', checkbox("defaults", 1, true, 'Default values', "columnShow(this.checked, 5)", "jsonly");
        ...
        echo (support("comment") ? "<label><input type='checkbox' name='comments' value='1' class='jsonly' onclick=\"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();\"" . ($cb ? " checked" : "") . ">" . 'Comment' . "</label>" . ' <input name="Comment" id="Comment" value="' . h($L["Comment"]) . '" maxlength="' . ($e->server_info >= 5.5 ? 2048 : 60) . '"' . ($cb ? '' : ' class="hidden"') . '>' : ''), '<p>
...
';
    ...
    if (support("partitioning")) {
        $xe = preg_match('~RANGE|LIST~', $L["partition_by"]);
        print_fieldset("partition", 'Partition by', $L["partition_by"]);
        echo '<p>
', "<select name='partition_by' onchange='partitionByChange(this);'" . on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1) . ">" . optionlist(array(
            "" => ""
        ) + $we, $L["partition_by"]) . "</select>", '(<input name="partition" value="', h($L["partition"]), '">)
Partitions: <input type="number" name="partitions" class="size', ($xe || !$L["partition_by"] ? " hidden" : ""), '" value="', h($L["partitions"]), '">
...
';
    ...
    if (preg_match('~MyISAM|M?aria' . ($e->server_info >= 5.6 ? '|InnoDB' : '') . '~i', $R["Engine"]))
        $Nc[] = "FULLTEXT";
    if (preg_match('~MyISAM|M?aria' . ($e->server_info >= 5.7 ? '|InnoDB' : '') . '~i', $R["Engine"]))
        $Nc[] = "SPATIAL";
    $w  = indexes($a);
    $Le = array();
    if ($y == "mongo") {
            ...
            if (in_array($v["type"], $Nc)) {
                ...
                if ($d) {
                    ...
                    if ($ec) {
                        ...
                        ksort($ec["lengths"]);
                        ksort($ec["descs"]);
                        if ($v["type"] == $ec["type"] && array_values($ec["columns"]) === $d && (!$ec["lengths"] || array_values($ec["lengths"]) === $od) && array_values($ec["descs"]) === $zb) {
                            unset($w[$E]);
                            continue;
                        }
                    }
...
<th><noscript><input type='image' class='icon' name='add[0]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.0&amp;driver=mysql' alt='+' title='Add next'></noscript>&nbsp;
</thead>
<?php
    if ($Le) {
        echo "<tr><td>PRIMARY<td>";
        foreach ($Le["columns"] as $z => $c) {
    ...
    foreach ($L["indexes"] as $v) {
        if (!$_POST["drop_col"] || $x != key($_POST["drop_col"])) {
            echo "<tr><td>" . html_select("indexes[$x][type]", array(
                -1 => ""
            ) + $Nc, $v["type"], ($x == count($L["indexes"]) ? "indexesAddRow(this);" : 1), "label-type"), "<td>";
            ksort($v["columns"]);
            $t = 1;
            foreach ($v["columns"] as $z => $c) {
                echo "<span>" . select_input(" name='indexes[$x][columns][$t]' onchange=\"" . ($t == count($v["columns"]) ? "indexesAddColumn" : "indexesChangeColumn") . "(this, '" . h(js_escape($y == "sql" ? "" : $_GET["indexes"] . "_")) . "');\" title='" . 'Column' . "'", ($m ? array_combine($m, $m) : $m), $c), ($y == "sql" || $y == "mssql" ? "<input type='number' name='indexes[$x][lengths][$t]' class='size' value='" . h($v["lengths"][$z]) . "' title='" . 'Length' . "'>" : ""), ($y != "sql" ? checkbox("indexes[$x][descs][$t]", 1, $v["descs"][$z], 'descending') : ""), " </span>";
                $t++;
            }
            echo "<td><input name='indexes[$x][name]' value='" . h($v["name"]) . "' autocapitalize='off' aria-labelledby='label-name'>\n", "<td><input type='image' class='icon' name='drop_col[$x]' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=cross.gif&amp;version=4.3.0&amp;driver=mysql' alt='x' title='" . 'Remove' . "' onclick=\"return !editingRemoveRow(this, 'indexes\$1[type]');\">\n";
        }
        $x++;
    }
    echo '</table>
    ...
    if ($_POST && !$k && !isset($_POST["add_x"])) {
        ...
        if ($_POST["drop"]) {
            ...
            if (DB != "") {
                ...
                restart_session();
                set_session("dbs", null);
                queries_redirect(ME . "db=" . urlencode($hd), 'Database has been created.', $Pf);
            }
        } else {
            if (!$L["collation"])
...
<?php
    ...
    elseif (!$_POST["add_x"] && $_GET["db"] == "")
        echo "<input type='image' class='icon' name='add' src='" . h(preg_replace("~\\?.*~", "", ME)) . "?file=plus.gif&amp;version=4.3.0&amp;driver=mysql' alt='+' title='" . 'Add next' . "'>\n";
    echo '<input type="hidden" name="token" value="', $T, '">
</form>
';
    ...
    $mf = routine($da, (isset($_GET["callf"]) ? "FUNCTION" : "PROCEDURE"));
    $Mc = array();
    $qe = array();
    foreach ($mf["fields"] as $t => $l) {
        if (substr($l["inout"], -3) == "OUT")
            $qe[$t] = "@" . idf_escape($l["field"]) . " AS " . idf_escape($l["field"]);
        if (!$l["inout"] || substr($l["inout"], 0, 2) == "IN")
            $Mc[] = $t;
    }
    if (!$k && $_POST) {
        $Ja = array();
        foreach ($mf["fields"] as $z => $l) {
            if (in_array($z, $Mc)) {
                $X = process_input($l);
                if ($X === false)
                    $X = "''";
            ...
            $Ja[] = (isset($qe[$z]) ? "@" . idf_escape($l["field"]) : $X);
        }
        ...
        if (!$e->multi_query($I))
            ...
            if ($qe)
                select($e->query("SELECT " . implode(", ", $qe)));
        }
    }
    echo '
<form action="" method="post">
...
';
    ...
    if ($_POST && !$k && !$_POST["add"] && !$_POST["change"] && !$_POST["change-js"]) {
        ...
        if ($y == "sqlite")
            ...
            if ($_POST["drop"])
                query_redirect($sa . $Gb, $B, $D);
            else {
                query_redirect($sa . ($E != "" ? "$Gb," : "") . "\nADD" . format_foreign_key($L), $B, $D);
    ...
    $Ff = array_keys(fields($a));
    $bg = ($a === $L["table"] ? $Ff : array_keys(fields($L["table"])));
...
';
        ...
        foreach ($L["source"] as $z => $X) {
            echo "<tr>", "<td>" . html_select("source[" . (+$z) . "]", array(
                -1 => ""
            ) + $Ff, $X, ($x == count($L["source"]) - 1 ? "foreignAddRow(this);" : 1), "label-source"), "<td>" . html_select("target[" . (+$z) . "]", $bg, $L["target"][$z], 1, "label-target");
            $x++;
        }
        echo '</table>
<p>
ON DELETE: ', html_select("on_delete", array(
            -1 => ""
        ) + explode("|", $Zd), $L["on_delete"]), ' ON UPDATE: ', html_select("on_update", array(
            -1 => ""
        ) + explode("|", $Zd), $L["on_update"]), doc_link(array(
...
';
    ...
    if ($_POST && !$k) {
        ...
        if (!$_POST["drop"] && $a == $E && $y != "sqlite" && $U == "VIEW" && $oe == "VIEW")
            query_redirect(($y == "mssql" ? "ALTER" : "CREATE OR REPLACE") . " VIEW " . table($E) . $ua, $B, $D);
        else {
            $dg = $E . "_adminer_" . uniqid();
            drop_create("DROP $oe " . table($a), "CREATE $U " . table($E) . $ua, "DROP $U " . table($E), "CREATE $U " . table($dg) . $ua, "DROP $U " . table($dg), ($_POST["drop"] ? substr(ME, 0, -1) : $B), 'View has been dropped.', $D, 'View has been created.', $a, $E);
        }
    }
    if (!$_POST && $a != "") {
        $L                 = view($a);
        $L["name"]         = $a;
        $L["materialized"] = ($oe != "VIEW");
        if (!$k)
            $k = error();
    }
...
';
    ...
    if ($_POST && !$k) {
        ...
        elseif (in_array($L["INTERVAL_FIELD"], $Sc) && isset($Kf[$L["STATUS"]])) {
            ...
            queries_redirect(substr(ME, 0, -1), ($aa != "" ? 'Event has been altered.' : 'Event has been created.'), queries(($aa != "" ? "ALTER EVENT " . idf_escape($aa) . $qf . ($aa != $L["EVENT_NAME"] ? "\nRENAME TO " . idf_escape($L["EVENT_NAME"]) : "") : "CREATE EVENT " . idf_escape($L["EVENT_NAME"]) . $qf) . "\n" . $Kf[$L["STATUS"]] . " COMMENT " . q($L["EVENT_COMMENT"]) . rtrim(" DO\n$L[EVENT_DEFINITION]", ";") . ";"));
        }
    }
...
<tr><th>Every<td><input type="number" name="INTERVAL_VALUE" value="', h($L["INTERVAL_VALUE"]), '" class="size"> ', html_select("INTERVAL_FIELD", $Sc, $L["INTERVAL_FIELD"]), '<tr><th>Status<td>', html_select("STATUS", $Kf, $L["STATUS"]), '<tr><th>Comment<td><input name="EVENT_COMMENT" value="', h($L["EVENT_COMMENT"]), '" maxlength="64">
...
';
    ...
    $L["fields"] = (array) $L["fields"];
    if ($_POST && !process_fields($L["fields"]) && !$k) {
        $dg = "$L[name]_adminer_" . uniqid();
        drop_create("DROP $mf " . idf_escape($da), create_routine($mf, $L), "DROP $mf " . idf_escape($L["name"]), create_routine($mf, array(
            "name" => $dg
        ) + $L), "DROP $mf " . idf_escape($dg), substr(ME, 0, -1), 'Routine has been dropped.', 'Routine has been altered.', 'Routine has been created.', $da, $L["name"]);
    }
...
<form action="" method="post" id="form">
<p>Name: <input name="name" value="', h($L["name"]), '" maxlength="64" autocapitalize="off">
', ($nf ? 'Language' . ": " . html_select("language", $nf, $L["language"]) : ""), '<input type="submit" value="Save">
<table cellspacing="0" class="nowrap">
';
...
';
    ...
    if ($_POST) {
        if (!$k && in_array($_POST["Timing"], $vg["Timing"]) && in_array($_POST["Event"], $vg["Event"]) && in_array($_POST["Type"], $vg["Type"])) {
            $Yd = " ON " . table($a);
            $Gb = "DROP TRIGGER " . idf_escape($E) . ($y == "pgsql" ? $Yd : "");
            $B  = ME . "table=" . urlencode($a);
            if ($_POST["drop"])
                query_redirect($Gb, $B, 'Trigger has been dropped.');
            else {
                if ($E != "")
                    queries($Gb);
                queries_redirect($B, ($E != "" ? 'Trigger has been altered.' : 'Trigger has been created.'), queries(create_trigger($Yd, $_POST)));
                if ($E != "")
                    queries(create_trigger($Yd, $L + array(
                        "Type" => reset($vg["Type"])
                    )));
            }
        }
...
<form action="" method="post" id="form">
<table cellspacing="0">
<tr><th>Time<td>', html_select("Timing", $vg["Timing"], $L["Timing"], "triggerChange(/^" . preg_quote($a, "/") . "_[ba][iud]$/, '" . js_escape($a) . "', this.form);"), '<tr><th>Event<td>', html_select("Event", $vg["Event"], $L["Event"], "this.form['Timing'].onchange();"), (in_array("UPDATE OF", $vg["Event"]) ? " <input name='Of' value='" . h($L["Of"]) . "' class='hidden'>" : ""), '<tr><th>Type<td>', html_select("Type", $vg["Type"], $L["Type"]), '</table>
<p>Name: <input name="Trigger" value="', h($L["Trigger"]);
...
';
    ...
    if (isset($_GET["host"]) && ($J = $e->query("SHOW GRANTS FOR " . q($fa) . "@" . q($_GET["host"])))) {
        while ($L = $J->fetch_row()) {
            if (preg_match('~GRANT (.*) ON (.*) TO ~', $L[0], $C) && preg_match_all('~ *([^(,]*[^ ,(])( *\\([^)]+\\))?~', $C[1], $ud, PREG_SET_ORDER)) {
                foreach ($ud as $X) {
                    if ($X[1] != "USAGE")
                        $zc["$C[2]$X[2]"][$X[1]] = true;
    ...
    if ($_POST && !$k) {
        $Xd = (isset($_GET["host"]) ? q($fa) . "@" . q($_GET["host"]) : "''");
        if ($_POST["drop"])
            query_redirect("DROP USER $Xd", ME . "privileges=", 'User has been dropped.');
        else {
            $Nd = q($_POST["user"]) . "@" . q($_POST["host"]);
            $_e = $_POST["pass"];
            if ($_e != '' && !$_POST["hashed"]) {
                $_e = $e->result("SELECT PASSWORD(" . q($_e) . ")");
                $k  = !$_e;
            }
            $lb = false;
            if (!$k) {
                if ($Xd != $Nd) {
                    $lb = queries(($e->server_info < 5 ? "GRANT USAGE ON *.* TO" : "CREATE USER") . " $Nd IDENTIFIED BY PASSWORD " . q($_e));
                    $k  = !$lb;
                } elseif ($_e != $Wd)
                    queries("SET PASSWORD FOR $Nd = " . q($_e));
            }
            if (!$k) {
                $jf = array();
                foreach ($Ld as $Rd => $r) {
                    ...
                    $r = array_keys($r);
                    if (isset($_GET["grant"]))
                        $jf = array_diff(array_keys(array_filter($Ld[$Rd], 'strlen')), $r);
                    elseif ($Xd == $Nd) {
                    ...
                    if (preg_match('~^(.+)\\s*(\\(.*\\))?$~U', $Rd, $C) && (!grant("REVOKE", $jf, $C[2], " ON $C[1] FROM $Nd") || !grant("GRANT", $r, $C[2], " ON $C[1] TO $Nd"))) {
                        $k = true;
                        break;
                    }
                }
...
';
    ...
    echo checkbox("hashed", 1, $L["hashed"], 'Hashed', "typePassword(this.form['pass'], this.checked);"), '</table>
...
';
    ...
    $t = 0;
    foreach ($zc as $Rd => $r) {
        echo '<th>' . ($Rd != "*.*" ? "<input name='objects[$t]' value='" . h($Rd) . "' size='10' autocapitalize='off'>" : "<input type='hidden' name='objects[$t]' value='*.*' size='10'>*.*");
        $t++;
    }
    echo "</thead>\n";
    foreach (array(
        ...
        foreach ((array) $Pe[$hb] as $Oe => $bb) {
            echo "<tr" . odd() . "><td" . ($yb ? ">$yb<td" : " colspan='2'") . ' lang="en" title="' . h($bb) . '">' . h($Oe);
            $t = 0;
            foreach ($zc as $Rd => $r) {
                $E = "'grants[$t][" . h(strtoupper($Oe)) . "]'";
                $Y = $r[strtoupper($Oe)];
                if ($hb == "Server Admin" && $Rd != (isset($zc["*.*"]) ? "*.*" : ".*"))
                    echo "<td>&nbsp;";
                elseif (isset($_GET["grant"]))
                    echo "<td><select name=$E><option><option value='1'" . ($Y ? " selected" : "") . ">" . 'Grant' . "<option value='0'" . ($Y == "0" ? " selected" : "") . ">" . 'Revoke' . "</select>";
                else
                    echo "<td align='center'><label class='block'><input type='checkbox' name=$E value='1'" . ($Y ? " checked" : "") . ($Oe == "All privileges" ? " id='grants-$t-all'" : ($Oe == "Grant option" ? "" : " onclick=\"if (this.checked) formUncheck('grants-$t-all');\"")) . "></label>";
                $t++;
            }
        }
    }
...
';
    ...
    echo "<h3>" . 'Master status' . doc_link(array(
        "sql" => "show-master-status.html"
    )) . "</h3>\n";
    $td = replication_status("MASTER");
    if (!$td)
    ...
    $Df = replication_status("SLAVE");
    if ($Df) {
        echo "<h3>" . 'Slave status' . doc_link(array(
            "sql" => "show-slave-status.html"
        )) . "</h3>\n";
        foreach ($Df[0] as $Cf) {
            echo "<table cellspacing='0'>\n";
            foreach ($Cf as $z => $X) {
    ...
    $he = $b->selectOrderProcess($m, $w);
    $_  = $b->selectLimitProcess();
    $xc = ($N ? implode(", ", $N) : "*" . ($Td ? ", $Td" : "")) . convert_fields($d, $m, $N) . "\nFROM " . table($a);
    $_c = ($s && $Wc ? "\nGROUP BY " . implode(", ", $s) : "") . ($he ? "\nORDER BY " . implode(", ", $he) : "");
    if ($_GET["val"] && is_ajax()) {
        header("Content-Type: text/plain; charset=utf-8");
        foreach ($_GET["val"] as $Cg => $L) {
    ...
    if ($_POST && !$k) {
        ...
        $Vg = ($Vg ? "\nWHERE " . implode(" AND ", $Vg) : "");
        $Le = $Eg = null;
        foreach ($w as $v) {
            if ($v["type"] == "PRIMARY") {
                $Le = array_flip($v["columns"]);
                $Eg = ($N ? $Le : array());
                break;
            }
        }
        foreach ((array) $Eg as $z => $X) {
        ...
        if ($_POST["export"]) {
            ...
            if (!is_array($_POST["check"]) || $Eg === array())
                ...
                foreach ($_POST["check"] as $X)
                    $Ag[] = "(SELECT" . limit($xc, "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $m) . $_c, 1) . ")";
                $I = implode(" UNION ALL ", $Ag);
            }
        ...
        if (!$b->selectEmailProcess($Z, $o)) {
            if ($_POST["save"] || $_POST["delete"]) {
                ...
                if ($_POST["delete"] || $P) {
                    ...
                    if ($_POST["all"] || ($Eg === array() && is_array($_POST["check"])) || $Wc) {
                        $J  = ($_POST["delete"] ? $j->delete($a, $Vg) : ($_POST["clone"] ? queries("INSERT $I$Vg") : $j->update($a, $P, $Vg)));
                        $na = $e->affected_rows;
                    } else {
                        foreach ((array) $_POST["check"] as $X) {
                            $Ug = "\nWHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($X, $m);
                            $J  = ($_POST["delete"] ? $j->delete($a, $Ug, 1) : ($_POST["clone"] ? queries("INSERT" . limit1($I, $Ug)) : $j->update($a, $P, $Ug)));
                            if (!$J)
                                break;
                            $na += $e->affected_rows;
                        }
                ...
                if ($_POST["clone"] && $J && $na == 1) {
                    $id = last_id();
                    if ($id)
                        $D = sprintf('Item%s has been inserted.', " $id");
                }
                ...
                if (!$_POST["val"])
                    ...
                    $J  = true;
                    $na = 0;
                    foreach ($_POST["val"] as $Cg => $L) {
                        $P = array();
                        foreach ($L as $z => $X) {
                            $z                 = bracket_escape($z, 1);
                            $P[idf_escape($z)] = (preg_match('~char|text~', $m[$z]["type"]) || $X != "" ? $b->processInput($m[$z], $X) : "NULL");
                        }
                        $J = $j->update($a, $P, " WHERE " . ($Z ? implode(" AND ", $Z) . " AND " : "") . where_check($Cg, $m), !($Wc || $Eg === array()), " ");
                        if (!$J)
                            break;
                        $na += $e->affected_rows;
                    }
            ...
            } elseif (!is_string($nc = get_file("csv_file", true)))
                $k = upload_error($nc);
            elseif (!preg_match('~~u', $nc))
                ...
                $M  = array();
                foreach ($ud[0] as $z => $X) {
                    preg_match_all("~((?>\"[^\"]*\")+|[^$wf]*)$wf~", $X . $wf, $vd);
                    if (!$z && !array_diff($vd[1], $Ya)) {
                        $Ya = $vd[1];
                        $na--;
                    } else {
                        $P = array();
                        foreach ($vd[1] as $t => $Ua)
                            $P[idf_escape($Ya[$t])] = ($Ua == "" && $m[$Ya[$t]]["null"] ? "NULL" : q(str_replace('""', '"', preg_replace('~^"|"$~', '', $Ua))));
                        $M[] = $P;
                    }
                }
                $J = (!$M || $j->insertUpdate($a, $M, $Le));
                if ($J)
                    $J = $j->commit();
    ...
    if (!$d && support("table"))
        ...
        $b->selectOrderPrint($he, $d, $w);
        $b->selectLimitPrint($_);
        $b->selectLengthPrint($gg);
        $b->selectActionPrint($w);
        echo "</form>\n";
        $F = $_GET["page"];
        if ($F == "last") {
            $wc = $e->result(count_rows($a, $Z, $Wc, $s));
            $F  = floor(max(0, $wc - 1) / $_);
        }
        $tf = $N;
        if (!$tf) {
            $tf[] = "*";
        ...
        if ($ib)
            $tf[] = substr($ib, 2);
        $J = $j->select($a, $tf, $Z, $s, $he, $_, $F, true);
        if (!$J)
            echo "<p class='error'>" . error() . "\n";
        else {
            if ($y == "mssql" && $F)
            ...
            if ($_GET["page"] != "last" && +$_ && $s && $Wc && $y == "sql")
                $wc = $e->result(" SELECT FOUND_ROWS()");
            if (!$M)
                echo "<p class='message'>" . 'No rows.' . "\n";
            else {
                $Ba = $b->backwardKeys($a, $Vf);
                echo "<table id='table' cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);' onkeydown='return editingKeydown(event);'>\n", "<thead><tr>" . (!$s && $N ? "" : "<td><input type='checkbox' id='all-page' onclick='formCheck(this, /check/);' class='jsonly'> <a href='" . h($_GET["modify"] ? remove_from_uri("modify") : $_SERVER["REQUEST_URI"] . "&modify=1") . "'>" . 'Modify' . "</a>");
                $Kd = array();
                $yc = array();
                reset($N);
                $Xe = 1;
                foreach ($M[0] as $z => $X) {
                    if ($z != $Td) {
                        ...
                        if ($E != "") {
                            ...
                            $yb     = "&desc%5B0%5D=1";
                            echo '<th onmouseover="columnMouse(this);" onmouseout="columnMouse(this, \' hidden\');">', '<a href="' . h($Ic . ($he[0] == $c || $he[0] == $z || (!$he && $Wc && $s[0] == $c) ? $yb : '')) . '">';
                            echo apply_sql_function($X["fun"], $E) . "</a>";
                            echo "<span class='column hidden'>", "<a href='" . h($Ic . $yb) . "' title='" . 'descending' . "' class='text'> â†“</a>";
                            if (!$X["fun"])
                ...
                if ($_GET["modify"]) {
                    foreach ($M as $L) {
                        foreach ($L as $z => $X)
                            $od[$z] = max($od[$z], min(40, strlen(utf8_decode($X))));
                    }
                }
                ...
                foreach ($b->rowDescriptions($M, $o) as $Jd => $L) {
                    $Bg = unique_array($M[$Jd], $w);
                    if (!$Bg) {
                        $Bg = array();
                        foreach ($M[$Jd] as $z => $X) {
                    ...
                    foreach ($Bg as $z => $X) {
                        ...
                        $Cg .= "&" . ($X !== null ? urlencode("where[" . bracket_escape($z) . "]") . "=" . urlencode($X) : "null%5B%5D=" . urlencode($z));
                    }
                    echo "<tr" . odd() . ">" . (!$s && $N ? "" : "<td>" . checkbox("check[]", substr($Cg, 1), in_array(substr($Cg, 1), (array) $_POST["check"]), "", "this.form['all'].checked = false; formUncheck('all-page');") . ($Wc || information_schema(DB) ? "" : " <a href='" . h(ME . "edit=" . urlencode($a) . $Cg) . "'>" . 'edit' . "</a>"));
                    foreach ($L as $z => $X) {
                        if (isset($Kd[$z])) {
                            $l = $m[$z];
                            if ($X != "" && (!isset($Sb[$z]) || $Sb[$z] != ""))
                                $Sb[$z] = (is_mail($X) ? $Kd[$z] : "");
                            $A = "";
                            if (preg_match('~blob|bytea|raw|file~', $l["type"]) && $X != "")
                                $A = ME . 'download=' . urlencode($a) . '&field=' . urlencode($z) . $Cg;
                            if (!$A && $X !== null) {
                                foreach ((array) $o[$z] as $n) {
                                    if (count($o[$z]) == 1 || end($n["source"]) == $z) {
                                        $A = "";
                                        foreach ($n["source"] as $t => $Ff)
                                            $A .= where_link($t, $n["target"][$t], $M[$Jd][$Ff]);
                            ...
                            if ($z == "COUNT(*)") {
                                ...
                                foreach ((array) $_GET["where"] as $W) {
                                    if (!array_key_exists($W["col"], $Bg))
                                        $A .= where_link($t++, $W["col"], $W["val"], $W["op"]);
                                }
                                foreach ($Bg as $ad => $W)
                                    $A .= where_link($t++, $ad, $W);
                            }
                            $X  = select_value($X, $A, $l, $gg);
                            $u  = h("val[$Cg][" . bracket_escape($z) . "]");
                            $Y  = $_POST["val"][$Cg][bracket_escape($z)];
                            $Nb = !is_array($L[$z]) && is_utf8($X) && $M[$Jd][$z] == $L[$z] && !$yc[$z];
                            $fg = preg_match('~text|lob~', $l["type"]);
                            if (($_GET["modify"] && $Nb) || $Y !== null) {
                                $Bc = h($Y !== null ? $Y : $L[$z]);
                                echo "<td>" . ($fg ? "<textarea name='$u' cols='30' rows='" . (substr_count($L[$z], "\n") + 1) . "'>$Bc</textarea>" : "<input name='$u' value='$Bc' size='$od[$z]'>");
                            } else {
                                $sd = strpos($X, "<i>...</i>");
                                echo "<td id='$u' onclick=\"selectClick(this, event, " . ($sd ? 2 : ($fg ? 1 : 0)) . ($Nb ? "" : ", '" . h('Use edit link to modify this value.') . "'") . ");\">$X";
                            }
                        }
                    }
                    if ($Ba)
                        echo "<td>";
                    $b->backwardKeysPrint($Ba, $M[$Jd]);
                    echo "</tr>\n";
                }
                if (is_ajax())
                    exit;
            ...
            if (($M || $F) && !is_ajax()) {
                ...
                if ($_GET["page"] != "last") {
                    if (!+$_)
                        $wc = count($M);
                    elseif ($y != "sql" || !$Wc) {
                        $wc = ($Wc ? false : found_rows($R, $Z));
                        if ($wc < max(1e4, 2 * ($F + 1) * $_))
                            $wc = reset(slow_query(count_rows($a, $Z, $Wc, $s)));
                        else
                            $cc = false;
                    }
                }
                if (+$_ && ($wc === false || $wc > $_ || $F)) {
                    echo "<p class='pages'>";
                    $xd = ($wc === false ? $F + (count($M) >= $_ ? 2 : 1) : floor(($wc - 1) / $_));
                    if ($y != "simpledb") {
                        echo '<a href="' . h(remove_from_uri("page")) . "\" onclick=\"pageClick(this.href, +prompt('" . 'Page' . "', '" . ($F + 1) . "'), event); return false;\">" . 'Page' . "</a>:", pagination(0, $F) . ($F > 5 ? " ..." : "");
                        for ($t = max(1, $F - 4); $t < min($xd, $F + 5); $t++)
                            echo pagination($t, $F);
                        if ($xd > 0) {
                            echo ($F + 5 < $xd ? " ..." : ""), ($cc && $wc !== false ? pagination($xd, $F) : " <a href='" . h(remove_from_uri("page") . "&page=last") . "' title='~$xd'>" . 'last' . "</a>");
                        }
                        echo (($wc === false ? count($M) + 1 : $wc - $F * $_) > $_ ? ' <a href="' . h(remove_from_uri("page") . "&page=" . ($F + 1)) . '" onclick="return !selectLoadMore(this, ' . (+$_) . ', \'' . 'Loading' . '...\');" class="loadmore">' . 'Load more data' . '</a>' : '');
                    } else {
                        echo 'Page' . ":", pagination(0, $F) . ($F > 1 ? " ..." : ""), ($F ? pagination($F, $F) : ""), ($xd > $F ? pagination($F + 1, $F) . ($xd > $F + 1 ? " ..." : "") : "");
                    }
                }
                echo "<p class='count'>\n", ($wc !== false ? "(" . ($cc ? "" : "~ ") . lang(array(
                    '%d row',
                    '%d rows'
                ), $wc) . ") " : "");
                $Cb = ($cc ? "" : "~ ") . $wc;
                echo checkbox("all", 1, 0, 'whole result', "var checked = formChecked(this, /check/); selectCount('selected', this.checked ? '$Cb' : checked); selectCount('selected2', this.checked || !checked ? '$Cb' : checked);") . "\n";
...
';
                ...
                if ($uc) {
                    ...
                    $re = $b->dumpOutput();
                    echo ($re ? html_select("output", $re, $ma["output"]) . " " : ""), html_select("format", $uc, $ma["format"]), " <input type='submit' name='export' value='" . 'Export' . "'>\n", "</div></fieldset>\n";
                }
                echo (!$s && $N ? "" : "<script type='text/javascript'>tableCheck();</script>\n");
            }
            if ($b->selectImportPrint()) {
                print_fieldset("import", 'Import', !$M);
            ...
            $b->selectEmailPrint(array_filter($Sb, 'strlen'), $d);
    ...
    $Jf = isset($_GET["status"]);
    page_header($Jf ? 'Status' : 'Variables');
    $Og = ($Jf ? show_status() : show_variables());
    if (!$Og)
        echo "<p class='message'>" . 'No rows.' . "\n";
    else {
        echo "<table cellspacing='0'>\n";
        foreach ($Og as $z => $X) {
            echo "<tr>", "<th><code class='jush-" . $y . ($Jf ? "status" : "set") . "'>" . h($z) . "</code>", "<td>" . nbsp($X);
        }
        echo "</table>\n";
    }
        ...
        foreach (table_status() as $E => $R) {
            ...
            if (!is_view($R)) {
                ...
                foreach ($Sf + array(
                    ...
                    if ($R[$z] != "") {
                        $X = format_number($R[$z]);
                        json_row("$z-$E", ($z == "Rows" && $X && $R["Engine"] == ($Hf == "pgsql" ? "table" : "InnoDB") ? "~ $X" : $X));
                        if (isset($Sf[$z]))
                            $Sf[$z] += ($R["Engine"] != "InnoDB" || $z != "Data_free" ? $R[$z] : 0);
    ...
    $ag = array_merge((array) $_POST["tables"], (array) $_POST["views"]);
    if ($ag && !$k && !$_POST["search"]) {
        $J = true;
        $D = "";
        ...
        if ($_GET["ns"] !== "") {
            ...
            if (!$Zf)
                ...
                echo "<table cellspacing='0' class='nowrap checkable' onclick='tableClick(event);' ondblclick='tableClick(event, true);'>\n", '<thead><tr class="wrap"><td><input id="check-all" type="checkbox" onclick="formCheck(this, /^(tables|views)\[/);" class="jsonly">', '<th>' . 'Table', '<td>' . 'Engine' . doc_link(array(
                    ...
                    'sql' => 'charset-mysql.html'
                )), '<td>' . 'Data Length' . $Db, '<td>' . 'Index Length' . $Db, '<td>' . 'Data Free' . $Db, '<td>' . 'Auto Increment' . doc_link(array(
                    'sql' => 'example-auto-increment.html'
                )), '<td>' . 'Rows' . $Db, (support("comment") ? '<td>' . 'Comment' . $Db : ''), "</thead>\n";
                $S = 0;
                foreach ($Zf as $E => $U) {
                    $Qg = ($U !== null && !preg_match('~table~i', $U));
                    $u  = h("Table-" . $E);
                    echo '<tr' . odd() . '><td>' . checkbox(($Qg ? "views[]" : "tables[]"), $E, in_array($E, $ag, true), "", "formUncheck('check-all');", "", $u), '<th>' . (support("table") || support("indexes") ? "<a href='" . h(ME) . "table=" . urlencode($E) . "' title='" . 'Show structure' . "' id='$u'>" . h($E) . '</a>' : h($E));
                    if ($Qg) {
                ...
                echo "<tr><td>&nbsp;<th>" . sprintf('%d in total', count($Zf)), "<td>" . nbsp($y == "sql" ? $e->result("SELECT @@storage_engine") : ""), "<td>" . nbsp(db_collation(DB, collations()));
                foreach (array(
                    "Data_length",
                    "Index_length",
                    "Data_free"
                ...
                if (!information_schema(DB)) {
                    $Mg = "<input type='submit' value='" . 'Vacuum' . "'" . on_help("'VACUUM'") . "> ";
                    $ee = "<input type='submit' name='optimize' value='" . 'Optimize' . "'" . on_help($y == "sql" ? "'OPTIMIZE TABLE'" : "'VACUUM OPTIMIZE'") . "> ";
                    echo "<fieldset><legend>" . 'Selected' . " <span id='selected'></span></legend><div>" . ($y == "sqlite" ? $Mg : ($y == "pgsql" ? $Mg . $ee : ($y == "sql" ? "<input type='submit' value='" . 'Analyze' . "'" . on_help("'ANALYZE TABLE'") . "> " . $ee . "<input type='submit' name='check' value='" . 'Check' . "'" . on_help("'CHECK TABLE'") . "> " . "<input type='submit' name='repair' value='" . 'Repair' . "'" . on_help("'REPAIR TABLE'") . "> " : ""))) . "<input type='submit' name='truncate' value='" . 'Truncate' . "'" . confirm() . on_help($y == "sqlite" ? "'DELETE'" : "'TRUNCATE" . ($y == "pgsql" ? "'" : " TABLE'")) . "> " . "<input type='submit' name='drop' value='" . 'Drop' . "'" . confirm() . on_help("'DROP TABLE'") . ">\n";
            ...
            if (support("routine")) {
                ...
                if ($of) {
                    ...
                    odd('');
                    foreach ($of as $L) {
                        echo '<tr' . odd() . '>', '<th><a href="' . h(ME) . ($L["ROUTINE_TYPE"] != "PROCEDURE" ? 'callf=' : 'call=') . urlencode($L["ROUTINE_NAME"]) . '">' . h($L["ROUTINE_NAME"]) . '</a>', '<td>' . h($L["ROUTINE_TYPE"]), '<td>' . h($L["DTD_IDENTIFIER"]), '<td><a href="' . h(ME) . ($L["ROUTINE_TYPE"] != "PROCEDURE" ? 'function=' : 'procedure=') . urlencode($L["ROUTINE_NAME"]) . '">' . 'Alter' . "</a>";
                    }
                    echo "</table>\n";
                }
            ...
            if (support("event")) {
                ...
                if ($M) {
                    ...
                    echo "</table>\n";
                    $ac = $e->result("SELECT @@event_scheduler");
                    if ($ac && $ac != "ON")
