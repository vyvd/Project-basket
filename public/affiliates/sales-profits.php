							...
							<div class="panel-heading panel-primary">
								...
								<div class="date-filter pull-right">
									<form method="post" action="data/set-filter">
										From <input id="start_date" type="date" name="start_date" value="<?php echo $start_date;?>"> to <input type="date" id="end_date" name="end_date" value="<?php echo $end_date;?>">
										<input type="hidden" name="redirect" value="../<?php echo pathinfo($_SERVER['PHP_SELF'], PATHINFO_FILENAME);?>">
										<input type="submit" id="filter-sales" class="btn btn-xs btn-primary" value="Filter">
									</form>
								</div>
							</div>
	...
	$(document).ready(function() {
		...
		function fetch_data(is_date_search, start_date = '', end_date = '') {
		   	var datatable = $('#sales').DataTable({
		        	...
		        	"ajax": {
		        		"url": "<?php echo $protocol.$_SERVER['HTTP_HOST'] ?>/affiliates/data/ajax-sales-profits.php",
		        		"type": "GET",
		        		"data": {
		        	...
		        	lengthMenu: [[10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000], [10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]],
		    		pageLength: 10,
		    		dom: 'lBfrtip',
			        buttons: [
			            {
		...
		function fetch_meta_boxes(start_date, end_date) {
			$.ajax({
				type: "POST",
				url: "<?php echo $protocol.$_SERVER['HTTP_HOST'] ?>/affiliates/data/ajax-sales-profits-mb.php",
				dataType: "json",
				data: {
		...
		fetch_meta_boxes(start_date, end_date);
		$('#sales').on('click', '.refund', function(e) {
			swal({
			  title: "Are you sure?",
			  text: "You will mark the order as refunded!",
			  type: "warning",
			  ...
			  cancelButtonText: "No, cancel pls!",
			  	...
			  	if (result.value) {
					...
					$.ajax({
						type: "POST",
						url: "<?php echo $protocol.$_SERVER['HTTP_HOST'] ?>/affiliates/data/ajax-sales-profits-refund.php",
						data : {
			    	...
			    	swal("Cancelled", "Your refund was not proceesed", "error");
			  	}
			});
		});
		$('#sales').on('click', '.delete', function(e) {
			swal({
			  title: "Are you sure?",
			  ...
			  cancelButtonText: "No, cancel pls!",
			  	...
			  	if (result.value) {
					...
					$.ajax({
						type: "POST",
						url: "<?php echo $protocol.$_SERVER['HTTP_HOST'] ?>/affiliates/data/ajax-sales-profits-delete.php",
						data : {
							affiliate_id: affiliate_id,
							id: id
						},
			    	...
			    	swal("Cancelled", "Your order was not deleted", "error");
			  	}
			});
		});
	});
