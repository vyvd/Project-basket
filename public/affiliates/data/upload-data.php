...
$link_url = filter_input(INPUT_POST, 'link_url', FILTER_SANITIZE_STRING);
$target_dir = "banners/";
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["file"]["tmp_name"], $target_file)) {
		$stmt = $mysqli->prepare("INSERT INTO ap_banners (filename, filetype, adsize, link) VALUES (?, ?, ?, ?)");
		$stmt->bind_param('ssss', $db_filename, $extension, $adsize, $link_url);
		$stmt->execute();
		$stmt->close();
    } else {
