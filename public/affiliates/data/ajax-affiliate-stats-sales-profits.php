...
$columns = array(
    ...
    array( 
        ...
        'dt' => 3,
        'formatter' => function( $d, $row ) {
            global $money_format, $currency_symbol, $affiliate_id, $mysqli;
            $product = $row['product'];
            $query = "SELECT void FROM ap_earnings WHERE affiliate_id = '$affiliate_id' AND product = '$product'";
    ...
    array(
        ...
        'formatter' => function( $d, $row ) {
           ...
           // var_dump($data);
            $var = '';
            if($data['void'] != '1'){ 
                $var = '<button data-affiliate="'.$row['affiliate_id'].'" data-id="'.$d.'" class="btn btn-sm btn-inverse refund-sales">Refund</button>';
            }
            $var .= '<buton data-affiliate="'.$row['affiliate_id'].'" data-id="'.$d.'" class="btn btn-sm btn-danger delete-sales">Delete</button';
            return $var;
        }
    ),
);
