...
$this->setControllers(array("course", "account", "moosend"));

$account = ORM::for_table("accounts")->select(["subActive", "subExpiryDate"])->find_one(CUR_ID_FRONT);

// count total courses for reporting
$update = ORM::for_table("accounts")->find_one(CUR_ID_FRONT);
...
if($account->subActive == "1" || ($account->subActive == "3" && ($account->subExpiryDate >= date("Y-m-d")))) {
    header('Location: '.SITE_URL.'dashboard/premium');
    exit;
}

if($this->account->checkUserType("NCFE")) {
    header('Location: '.SITE_URL.'dashboard/ncfe');
    exit;
}

if($this->account->checkUserType("tutor")) {
    header('Location: '.SITE_URL.'dashboard/tutor');
    exit;
}

if($this->account->checkUserType("IQA")) {
    header('Location: '.SITE_URL.'dashboard/iqa');
    exit;
}
            ...
            <div class="row" v-else>
                ...
                <div class="col-12 regular-full">
                    <div class="row align-items-center">
                        ...
                        <div class="col-12 col-md-12 col-lg-4 text-center"><!-- xo-bg -->
                                ...
                                <div class="col-12 regular-full popularCourseboxes">
                                    <div class="row">
                                        ...
                                        foreach($items as $item) {
                                            $course = ORM::for_table("courses")->where("hidden", "0")->find_one($item->course_id);
    ...
    <script>
        var app = new Vue({
            ...
            beforeMount: function () {
                <?php
                if(($this->user->oldID != "" || $this->user->oldID != null) && ($this->user->dataImported == 0) && ($this->user->usImport == '1')){
                ?>
                that = this;
                that.importUserData();
                <?php
