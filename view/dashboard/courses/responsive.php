            ...
            <div v-else class="row">
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-12 col-md-12 col-lg-4 no-padding courseFilterHover <?php if($this->get["filter"] == "active") { ?>active<?php } ?>" onclick="parent.location='<?= SITE_URL ?>dashboard/courses?filter=active'">
                            <div class="white-rounded dash-details">
                                <h1 class="text-center">
                                    <?php
                                        $activeCoursesCount = ORM::for_table("coursesAssigned")
                                            ->where_null("bundleID")
                                            ->where("accountID", CUR_ID_FRONT);

                                        if($this->isActiveSubscription(CUR_ID_FRONT) === false){
                                            $activeCoursesCount = $activeCoursesCount->where("sub", '0');
                                        }else{
                                            $activeCoursesCount = $activeCoursesCount->where("completed", '0');
                                        }

                                        echo $activeCoursesCount->count();
                                    ?>
                                </h1>
                        ...
                        <div class="col-12 col-md-12 col-lg-4 no-padding courseFilterHover <?php if($this->get["filter"] == "") { ?>active<?php } ?>" onclick="parent.location='<?= SITE_URL ?>dashboard/courses'">
                            <div class="white-rounded dash-details">
                                <h1 class="text-center">
                                    <?php
                                    $allCoursesCount = ORM::for_table("coursesAssigned")
                                        ->where_null("bundleID")
                                        ->where("accountID", CUR_ID_FRONT);

                                    if($this->isActiveSubscription(CUR_ID_FRONT) === false){
                                        $allCoursesCount = $allCoursesCount->where_any_is(array(
                                            array('sub' => '0'),
                                            array('sub' => '1', 'completed' => '1')));
                                    }

                                    echo $allCoursesCount->count();
                                    ?>
                                </h1>
                ...
                <div class="col-12 regular-full popularCourseboxes">
                    <div class="row">
                        ...
                        <div  v-else-if="userCourses" v-for="(course, index) in userCourses" class="col-12 col-md-6 col-lg-6 col-xl-4">
